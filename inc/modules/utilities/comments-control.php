<?php
/**
 * PSToolkit Comments Control class.
 *
 * @package PSToolkit
 * @subpackage Utilites
 */
if ( ! class_exists( 'PSToolkit_Comments_Control' ) ) {

	class PSToolkit_Comments_Control extends PSToolkit_Helper {
		...
		public function __construct() {
			...
			add_filter( 'pstoolkit_settings_comments_control', array( $this, 'admin_options_page' ) );
			add_filter( 'pstoolkit_settings_comments_control_process', array( $this, 'update' ) );
			...
			add_filter( 'pstoolkit_settings_comments_control_preserve', array( $this, 'add_preserve_fields' ) );
		}

		/**
		 * Kommentare offen Filter
		 * // TODO: Irgendwann sollten wir diese Klasse umgestalten, um eine einzige Methode zum Überprüfen des Kommentarstatus zu haben.
		 *
		 * @since 2.2.0
		 */
		public function comments_open_check( $status, $post_id ) {
			...
			if (
				is_admin() // Der Admin-Bereich ist mir egal
				|| 'off' === $value // Keine Einschränkungen für Kommentare
				|| ! $status // Bereits deaktiviert, sodass wir nichts tun müssen
			) {
				return $status;
			}
			...
			$disabled_for_post_type = $by_post_type === 'off'
				? true // Für alle deaktiviert
				: pstoolkit_get_array_value( $disabled_post_types, $post_type ) === 'on'; // Für aktuellen Typ deaktiviert
		...
		public function set() {
			...
			// Diese set () -Methode entfernt möglicherweise die Unterstützung für Post-Typen für Kommentare
			// Speichere unveränderte Beitragstypen für später
			...
			$by_post_type = $this->get_value( 'settings', 'by_post_type', 'off' );
			$by_blacklist = $this->get_value( 'settings', 'by_blacklist', 'on' ); // "on" bedeutet, dass die Option "IP Blacklist" deaktiviert ist ¯\_(ツ)_/¯
			if ( 'off' === $by_post_type ) { // Deaktivieren für alle Beitragstypen
				if ( 'on' === $by_blacklist ) { // für alle IP deaktivieren
			...
			if ( $this->disallow_comments ) {
				...
				// Die Filterung der Admin-Leiste muss hier seit WP 3.6 erfolgen
		...
		public function upgrade_options() {
			...
			if ( empty( $value ) ) {
				$value['rules']['whitelist']    = pstoolkit_get_option( 'limit_comments_allowed_ips' );
				$value['settings']['blacklist'] = pstoolkit_get_option( 'limit_comments_denied_ips' );
				pstoolkit_update_option( $this->option_name, $value );
				pstoolkit_delete_option( 'limit_comments_allowed_ips' );
				pstoolkit_delete_option( 'limit_comments_denied_ips' );
			}
			$option_name_cpt = 'ub_comments_control_cpt';
			$value           = pstoolkit_get_option( $option_name_cpt );
			if ( ! empty( $value ) ) {
				$this->set_value( 'settings', 'available_post_types', $value );
				pstoolkit_delete_option( $option_name_cpt );
			}
		}
		...
		/**
		 * Optionen festlegen Konfiguration
		 *
		 * @since 1.8.6
		 */
		protected function set_options() {
			$description = __( 'Du kannst Kommentare auf Deiner Webseite global deaktivieren.', 'ub' );
			if ( $this->is_network ) {
				$description = __( 'Du kannst Kommentare in Deinem Netzwerk global deaktivieren.', 'ub' );
			}
			$options = array(
				'settings' => array(
					'title'       => __( 'Kommentare', 'ub' ),
					...
					'fields'      => array(
						...
						'by_post_type' => array(
							'type'         => 'sui-tab',
							'label'        => __( 'Beitragstyp', 'ub' ),
							'description'  => __( 'Wähle Beitragstyp, um Kommentare zu deaktivieren.', 'ub' ),
							'options'      => array(
								'off' => __( 'Alle', 'ub' ),
								'on'  => __( 'Bestimmte Beitragstypen', 'ub' ),
							),
							'default'      => 'off',
							'master'       => 'enabled',
						...
						'blacklist'    => array(
							'type'         => 'textarea',
							'description'  => __( 'Gib eine IP-Adresse pro Zeile ein. Sowohl IPv4 als auch IPv6 werden unterstützt. IP-Bereiche werden auch im Format xxx.xxx.xxx.xxx-xxx.xxx.xxx.xxx akzeptiert.', 'ub' ),
							'classes'      => array( 'large-text' ),
							'master'       => 'enabled-blacklist',
							'placeholder'  => esc_html__( 'Gib hier Deine IP-Blacklist ein...', 'ub' ),
							'display'      => 'sui-tab-content',
							'master-value' => 'off',
						),
						'by_blacklist' => array(
							'type'         => 'sui-tab',
							'label'        => __( 'Kommentare deaktivieren für', 'ub' ),
							'description'  => __( 'Du kannst Kommentare für alle deaktivieren oder Kommentare nur für IPs deaktivieren, die in einer IP-Blacklist aufgeführt sind.', 'ub' ),
							'options'      => array(
								'on'  => __( 'Jeder', 'ub' ),
						...
						'status'       => array(
							'type'        => 'sui-tab',
							'options'     => array(
								'off' => __( 'Aktivieren', 'ub' ),
								'on'  => __( 'Deaktivieren', 'ub' ),
							),
							'default'     => 'off',
							'slave-class' => 'enabled',
						),
					...
					'fields'  => array(
						'whitelist' => array(
							...
							'description-extra' => array(
								__( 'IPs, für die Kommentare nicht gedrosselt werden. Eine IP pro Zeile oder Komma getrennt.', 'ub' ),
							),
							'classes'           => array( 'large-text' ),
							'placeholder'       => esc_html__( 'Gib hier Deine IP-Whitelist ein...', 'ub' ),
						),
					),
					'master'  => array(
						'section' => 'settings',
		...
		public function filter_query() {
			if ( is_comment_feed() ) {
				wp_die( __( 'Kommentarfunktion ist geschlossen.', 'ub' ), '', array( 'response' => 403 ) );
			}
		}
		...
		public function setup_notice() {
			...
			$message = sprintf( __( 'Das <em>Kommentarsteuerung</em>-Modul ist aktiv, aber nicht richtig konfiguriert. Besuche die <a href="%s">Konfigurationsseite</a>, um auszuwählen, zu welchen Beitragstypen Kommentare deaktiviert werden sollen.', 'ub' ), esc_url( $url ) );
...
new PSToolkit_Comments_Control();
