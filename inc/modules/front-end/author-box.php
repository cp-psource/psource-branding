<?php
/**
 * PSToolkit Author Box class.
 *
 * @package PSToolkit
 * @subpackage Front-end
 */
if ( ! class_exists( 'PSToolkit_Author_Box' ) ) {
	...
	class PSToolkit_Author_Box extends PSToolkit_Helper {
		...
		public function __construct() {
			 ...
			 * Admin area
			 */
			add_filter( 'pstoolkit_settings_author_box', array( $this, 'admin_options_page' ) );
			add_filter( 'pstoolkit_settings_author_box_process', array( $this, 'update' ) );
			 ...
			 * @since 1.0.0
			 */
			add_filter( 'pstoolkit_get_options_fields_design_social', array( $this, 'add_design_social_position' ), 10, 3 );
			/**
			 * upgrade options
			 *
			 * @since 1.0.0
		...
		public function add_design_social_position( $data, $defaults, $module ) {
			...
			$data['social_media_position'] = array(
				...
				'options'   => array(
					'bottom'       => __( 'Unten', 'ub' ),
					'top'          => __( 'Oben', 'ub' ),
					'under-avatar' => __( 'Unter Avatar', 'ub' ),
				),
		...
		protected function set_options() {
			...
			$options = array(
				'show'    => array(
					'title'       => __( 'Konfigurieren', 'ub' ),
					'description' => __( 'Konfiguriere die Anzeigeoption und die Sichtbarkeit des Autorenfelds.', 'ub' ),
					'fields'      => array(
						'widget'    => array(
							'type'  => 'description',
							'label' => __( 'Autorenbox Widget', 'ub' ),
							'value' => sprintf(
								__( 'Gehe zu <b>Darstellung &gt; <a href="%s">Widgets</a></b> um das Autorenfeld-Widget in die Seitenleisten Deines Themes einzufügen. Alle in diesem Modul vorgenommenen Anpassungen wirken sich sowohl auf das Autorenfeld-Widget als auch auf das Inline-Inhaltsautorfeld aus.', 'ub' ),
								admin_url( 'widgets.php' )
							),
						),
						'mode'      => array(
							'type'        => 'sui-tab',
							'label'       => __( 'Inline-Inhalt', 'ub' ),
							'options'     => array(
								'on'  => __( 'Aktivieren', 'ub' ),
								'off' => __( 'Deaktivieren', 'ub' ),
							),
							'default'     => 'on',
							'description' => __( 'Du kannst das Autorenfeld inline in den Inhalt von Beiträgen, Seiten usw. einfügen. Deaktiviere diese Option, wenn Du nur das Autorenfeld-Widget verwenden möchtest.', 'ub' ),
						),
						'post_type' => array(
							'type'        => 'checkboxes',
							...
							'options'     => $post_types,
							'multiple'    => true,
							'description' => __( 'Wähle die Beitragstypen aus, für die das Autorenfeld angezeigt werden soll. Diese Optionen wirken sich auf die Sichtbarkeit des Autorenfeld-Widgets sowie des Inline-Autorenfelds aus.', 'ub' ),
							'default'     => array( 'post' ),
						),
					),
				),
				'profile' => array(
					'title'       => __( 'Autorenprofil', 'ub' ),
					'description' => __( 'Profile sind auf der Benutzerbearbeitungs- und Benutzerprofilseite verfügbar.', 'ub' ),
					'show-as'     => 'accordion',
				...
				'design'  => array(
					'title'       => __( 'Design', 'ub' ),
					'description' => __( 'Passe das Design der Autorenbox an.', 'ub' ),
					'show-as'     => 'accordion',
					'fields'      => $this->get_options_fields( 'design', array( 'name_and_bio', 'avatar', 'latests_entries', 'social', 'container', 'reset' ) ),
				),
				'colors'  => array(
					'title'       => __( 'Farben', 'ub' ),
					'description' => __( 'Passe die Standardfarbkombinationen des Autorenfelds an.', 'ub' ),
					'show-as'     => 'accordion',
		...
		public function add_social_media( $profileuser ) {
			...
			foreach ( $data as $key ) {
				...
				$fields[ $key ] = array(
					...
					'placeholder' => sprintf( esc_attr__( 'Gib hier %s URL ein...', 'ub' ), $label ),
				);
			}
			if ( empty( $fields ) ) {
				return;
		...
		private function box( $content = '' ) {
			...
			if ( 'on' == $show ) {
				...
				$box_content .= sprintf( '<div class="pstoolkit-author-box-avatar" style="min-width: %dpx;">', $size );
			...
			if ( 'on' === $show ) {
				...
				if ( $the_query->have_posts() ) {
					...
					while ( $the_query->have_posts() ) {
						...
						if ( empty( $title ) ) {
							$title = sprintf(
								'<small>[%s]</small>',
								esc_html__( 'Eintrag hat keinen Titel', 'ub' )
							);
						}
						$entries .= sprintf(
				...
				wp_reset_query();
				if ( ! empty( $entries ) ) {
					$part .= '<div class="pstoolkit-author-box-more">';
			 ...
			 */
			if ( ! empty( $part ) ) {
				$box_content .= sprintf( '<div class="pstoolkit-author-box-desc">%s</div>', $part );
			}
			/**
			 * wrap box content
			 */
			if ( ! empty( $box_content ) ) {
				$box_content = sprintf( '<div class="pstoolkit-author-box-content">%s</div>', $box_content );
			}
			/**
			 * social_media
			 */
			 ...
			 */
			if ( ! empty( $box_content ) ) {
				$content .= sprintf( '<div class="pstoolkit-author-box">%s</div>', $box_content );
			}
			return $content;
		}
				...
				if ( $value ) {
					...
					switch ( $key ) {
						case 'wp-profile-website':
							...
							$social[ $key ] = array( 'label' => __( 'Webseite', 'ub' ) );
							break;
						case 'mail':
							$value = 'mailto:' . $value;
							break;
		...
		protected function get_options_fields_design_name_and_bio( $defaults ) {
			$data = array(
				'name_and_bio_link_name'        => array(
					...
					'description'  => array(
						'content'  => __( 'Aktiviere diese Option, um den Namen des Autors mit dem Autorenarchiv zu verknüpfen.', 'ub' ),
						'position' => 'bottom',
					),
					'options'      => array(
						'off' => __( 'Deaktivieren', 'ub' ),
				...
				'name_and_bio_show_counter'     => array(
					'type'         => 'sui-tab',
					'label'        => __( 'Anzahl der Beiträge', 'ub' ),
					'description'  => array(
						'content'  => __( 'Dies zeigt die Anzahl der Beiträge des Autors zusammen mit dem Namen.', 'ub' ),
						'position' => 'bottom',
					),
					'options'      => array(
				...
				'name_and_bio_show_description' => array(
					'type'      => 'sui-tab',
					'label'     => __( 'Biografie des Autors', 'ub' ),
					'options'   => array(
						'off' => __( 'Ausblenden', 'ub' ),
						'on'  => __( 'Anzeigen', 'ub' ),
					),
			...
			$data = array(
				...
				'avatar_border_width'  => array(
					...
					'before_field' => sprintf(
						'<span class="sui-label">%s</span><div class="sui-border-frame pstoolkit-author-box-avatar-border"><div class="sui-row"><div class="sui-col">',
						esc_html__( 'Rahmen', 'ub' )
					),
					'after_field'  => '</div>',
				),
				'avatar_border_style'  => array(
					...
					'after_field'  => sprintf(
						...
						esc_html__( 'Farbe des Rahmens in der Dropdown-Liste Farbschema.', 'ub' )
					),
				),
				'avatar_show'          => array(
					'type'        => 'sui-tab',
		...
		protected function get_options_fields_design_latests_entries( $defaults ) {
			$data = array(
				'latests_entries_type'            => array(
					'type'         => 'sui-tab',
					'label'        => __( 'Eintragstyp', 'ub' ),
					'description'  => array(
						'content'  => __( 'Wenn sich ein Betrachter in einem Beitrag befindet, werden nur die Beiträge des Autors angezeigt. Seiten und Medientyp werden nicht berücksichtigt.', 'ub' ),
						'position' => 'bottom',
					),
					'options'      => array(
						'on'  => __( 'Entspricht dem aktuellen Beitragstyp', 'ub' ),
						'off' => __( 'Alle', 'ub' ),
					),
					'default'      => 'on',
				...
				'latests_entries_limit'           => array(
					'type'         => 'number',
					'label'        => __( 'Anzahl der Einträge', 'ub' ),
					'default'      => 5,
					'min'          => 1,
					'master'       => $this->get_name( 'entries' ),
					'master-value' => 'on',
				...
				'latests_entries_title'           => array(
					'label'        => __( 'Eintragstitel (optional)', 'ub' ),
					'description'  => array(
						'content'  => __( 'Lasse dieses Feld leer, wenn Du keinen Titel anzeigen möchtest.', 'ub' ),
						'position' => 'bottom',
					),
					'default'      => __( 'Lese mehr vom selben Autor:', 'ub' ),
					'master'       => $this->get_name( 'entries' ),
					'master-value' => 'on',
					'display'      => 'sui-tab-content',
				),
		...
		protected function get_options_fields_colors_social( $defaults ) {
			$data = array(
				...
				'social_monochrome'       => array(
					'type'        => 'color',
					'label'       => __( 'Einfarbig', 'ub' ),
					'description' => array(
						'content'  => __( 'Diese Farbe wird NUR verwendet, wenn die Symbole monochrom sind.', 'ub' ),
						'position' => 'bottom',
					),
					'default'     => '#222',
					'data'        => array(
		...
		protected function get_options_fields_content_social( $defaults = array() ) {
			...
			$data[ $key ]['before_field']            = '<div class="sui-box-builder-fields social-logo-color pstoolkit-social-logos-main-container">';
		...
		private function get_ace_selectors() {
			$selectors = array(
				'general' => array(
					'selectors' => array(
						'.pstoolkit-author-box-more'          => __( 'Box', 'ub' ),
						'.pstoolkit-author-box-desc h4'       => __( 'Autorenname', 'ub' ),
						'.pstoolkit-author-box-content'       => __( 'Box-Inhalt', 'ub' ),
						'.pstoolkit-author-box-more'          => __( 'Liste', 'ub' ),
						'.pstoolkit-author-box-avatar'        => __( 'Avatar', 'ub' ),
						'.pstoolkit-author-box .social-media' => __( 'Soziale Medien', 'ub' ),
					),
				),
			);
			return $selectors;
