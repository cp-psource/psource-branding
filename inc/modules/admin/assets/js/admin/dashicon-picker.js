jQuery(document).ready(function ($) {
	...
	window.PSToolkit.Dashicon_Picker = function ($input, icon) {
		var $el;
		...
		function init_search() {
			$('.sui-form-control', $el).on('input propertychange', _.debounce(function () {
				var $input_field = $(this),
					...
					$groups_and_icons = $('.pstoolkit-dashicon-picker-group, .dashicons', $el);
				...
				$matches.each(function () {
					$(this).show();
					$(this).closest('.pstoolkit-dashicon-picker-group').show();
				});
			}, 100));
		}
