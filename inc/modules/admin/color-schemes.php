 ...
 */
if ( ! class_exists( 'PSToolkit_Color_Schemes' ) ) {

	class PSToolkit_Color_Schemes extends PSToolkit_Helper {
		...
		public function __construct() {
			 ...
			 * Common module hooks
			 */
			add_filter( 'pstoolkit_settings_color_schemes', array( $this, 'admin_options_page' ) );
			add_filter( 'pstoolkit_settings_color_schemes_process', array( $this, 'update' ), 10 );
			 ...
			 * @since 1.0.0
			 */
			add_filter( 'pstoolkit_settings_color_schemes_preserve', array( $this, 'add_preserve_fields' ) );
			/**
			 ...
			 * AJAX
			 */
			add_action( 'wp_ajax_pstoolkit_color_schemes_save', array( $this, 'ajax_save' ) );
			/**
			 * upgrade options
			 *
			 * @since 1.0.0
			 ...
			 */
			add_filter( 'pstoolkit_admin_bar_logo_css_args', array( $this, 'bar_logo_args' ), 10, 1 );
		}
		...
		public function upgrade_options() {
			...
			$ucs_visible_color_schemes = pstoolkit_get_option( 'ucs_visible_color_schemes' );
			...
			$data = array(
				...
				'defaul_color_scheme'     => array(
					'scheme' => pstoolkit_get_option( 'ucs_default_color_scheme', 'do-not-change' ),
				),
				'force_color_scheme'      => array(
					'scheme' => pstoolkit_get_option( 'ucs_force_color_scheme', 'do-not-force' ),
				),
				'settings'                => array(
			 ...
			 * Delete old values
			 */
			pstoolkit_delete_option( 'ucs_default_color_scheme' );
			pstoolkit_delete_option( 'ucs_force_color_scheme' );
			pstoolkit_delete_option( 'ucs_visible_color_schemes' );
			/**
		 ...
		 *
		 * Convert Ultimate Scheme for version less than 2.0.0
		 *
		 * @since 1.0.0
		 */
		private function convert_ultimate_scheme() {
			...
			foreach ( $map as $old_name => $name ) {
				...
				$value = pstoolkit_get_option( $old_name, $default );
				/**
				 * delete old value
				 */
		...
		protected function set_options() {
			 ...
			 */
			$colors_force  = array(
				'do-not-force' => __( 'Farbschema nicht erzwingen', 'ub' ),
			);
			$colors_force += $colors;
			/**
			 * Options
			...
			$options = array(
				'available_color_schemes' => array(
					'title'       => __( 'Verfügbare Farbschema', 'ub' ),
					'description' => __( 'Wähle die Farbschemata aus, die im Benutzerprofil sichtbar sein sollen.', 'ub' ),
					'fields'      => array(
						'schemes' => array(
							'type'     => 'callback',
				...
				'defaul_color_scheme'     => array(
					'title'       => __( 'Standardfarbschema', 'ub' ),
					'description' => __( 'Wähle ein Standardfarbschema für neu registrierte Benutzer.', 'ub' ),
					'fields'      => array(
						'scheme' => array(
							'type'    => 'select',
				...
				'force_color_scheme'      => array(
					'title'       => __( 'Farbschema erzwingen', 'ub' ),
					'description' => __( 'Wähle ein Farbschema, das für jeden Benutzer verwendet werden soll.', 'ub' ),
					'fields'      => array(
						'scheme' => array(
							'type'    => 'select',
			...
			if ( $this->is_network ) {
				$options['force_color_scheme']['description'] = __( 'Wähle ein Farbschema, das für jeden Benutzer im Netzwerk verwendet werden soll.', 'ub' );
			}
		...
		public function admin_custom_color_scheme_option() {
			...
			// Set PSToolkit color scheme without colors and icons for including it in $_wp_admin_css_colors for set_option method
			// It will be updated in the end of this method
			wp_admin_css_color( $this->slug, 'PSToolkit', $admin_url );
		...
		public function get_list() {
			...
			if ( isset( $css_schemes[ $this->slug ] ) && array_keys( $css_schemes )[0] !== $this->slug ) {
				// move PSToolkit scheme css to the top of the list
				$pstoolkit_scheme = $css_schemes[ $this->slug ];
				unset( $css_schemes[ $this->slug ] );
				$css_schemes = array_merge( array( $this->slug => $pstoolkit_scheme ), $css_schemes );
			}
				...
				if ( $this->slug === $color ) {
					$args     = array(
						...
						'data'      => array(
							...
							'tooltip'    => esc_attr( sprintf( _x( 'Passe das Schema "%s" an', 'Label for link to edit custom theme', 'ub' ), $color_info->name ) ),
						),
						'sui'       => array(
							'tooltip',
						),
						'classes'   => array(
							'pstoolkit-customize-ultimate-scheme',
							'sui-tooltip',
						),
					);
		...
		public function ajax_save() {
			...
			$request = $this->sanitize_request_payload( $_POST['pstoolkit'] );
			$data    = array();
			foreach ( $this->defaults as $key => $default ) {
			...
			$message = array(
				'type'    => 'success',
				'message' => sprintf(
					__( '%s wurde erfolgreich aktualisiert.', 'ub' ),
