<?php
if ( ! class_exists( 'PSToolkit_Base' ) ) {
	include_once dirname( __FILE__ ) . '/class-pstoolkit-loader.php';
	class PSToolkit_Base extends PSToolkit_Loader {
		/**
		 * base URL
		 *
		 * @since 2.1.0
		...
		public function __construct() {
			 ...
			 * @since 1.0.0
			 */
			add_filter( 'pstoolkit_version', array( $this, 'version' ) );
		}
		...
		/**
		 * Get relevant chapter to PSToolkit Docs
		 *
		 * @since 3.2.0
		 * @return string
		 */
		...
		protected static function get_template_file_name( $key ) {
			$template_file = pstoolkit_dir( 'views/' . $key ) . '.php';
		...
		protected function can_load_module( $module ) {
			if (
				...
				return apply_filters( 'pstoolkit_can_load_module', false, $module );
			}
			if (
				is_multisite()
			...
			return apply_filters( 'pstoolkit_can_load_module', true, $module );
		}
	}
}
