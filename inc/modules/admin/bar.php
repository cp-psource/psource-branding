 ...
 */
if ( ! class_exists( 'PSToolkit_Admin_Bar' ) ) {

	/**
	 * Class PSToolkit_Admin_Bar.
	 */
	class PSToolkit_Admin_Bar extends PSToolkit_Helper {
		...
		/**
		 * PSToolkit_Admin_Bar constructor.
		 */
		public function __construct() {
			...
			$this->roles['guest'] = __( 'Gast', 'ub' );
			// Is network
			$this->item_defaults['is_network'] = $this->is_network;
			// Available roles
			$this->item_defaults['available_roles'] = $this->roles;
			// Common module hooks.
			add_filter( 'pstoolkit_settings_admin_bar', array( $this, 'admin_options_page' ) );
			add_filter( 'pstoolkit_settings_admin_bar_process', array( $this, 'update' ), 10 );
			add_filter( 'pstoolkit_settings_admin_bar_process', array( $this, 'validate_builders' ), 11 );
			add_filter( 'pstoolkit_settings_admin_bar_preserve', array( $this, 'add_preserve_fields' ) );
			...
			add_action( 'wp_ajax_pstoolkit_admin_bar_order_save', array( $this, 'ajax_order_save' ) );
			add_action( 'wp_ajax_pstoolkit_admin_bar_order_reset', array( $this, 'ajax_order_reset' ) );
			/**
			 * Admin bar items
			 */
			add_action( 'wp_ajax_pstoolkit_admin_bar_menu_save', array( $this, 'ajax_save_item' ) );
			add_action( 'wp_ajax_pstoolkit_admin_bar_delete', array( $this, 'ajax_delete_item' ) );
			add_action( 'wp_ajax_pstoolkit_admin_bar_submenu_restore', array( $this, 'ajax_submenu_restore' ) );
			/**
			 * Since 3.0.7
			 */
			add_action( 'wp_ajax_pstoolkit_admin_bar_get', array( $this, 'ajax_get_item' ) );
			...
			/**
			 * PSToolkit admin enqueue scripts
			 *
			 * @since 3.1.0
			 */
			add_action( 'pstoolkit_admin_enqueue_module_admin_assets', array( $this, 'enqueue_scripts' ) );
			/**
			 ...
			 * @since 3.2
			 */
			add_action( 'pstoolkit_helper_update', array( $this, 'reset_cache' ), 10, 3 );
		}
		...
		public function upgrade_options() {
			...
			$update = false;
			// Admin Bar Logo.
			$data = pstoolkit_get_option( 'admin_bar_logo' );
			if ( ! empty( $data ) ) {
				...
				pstoolkit_delete_option( 'admin_bar_logo' );
			}
			// Custom Admin Bar: custom menus.
			$data = pstoolkit_get_option_filtered( 'ub_admin_bar_menus' );
			if ( ! empty( $data ) && is_array( $data ) ) {
				...
				foreach ( $data as $menu_id ) {
					...
					$menu        = maybe_unserialize( pstoolkit_get_option( $option_name ) );
					if ( empty( $menu ) ) {
						pstoolkit_delete_option( $option_name );
						continue;
					}
				...
				pstoolkit_delete_option( 'ub_admin_bar_menus' );
			}
			// Custom Admin Bar: custom order.
			$data = pstoolkit_get_option_filtered( 'ub_admin_bar_order' );
			if ( ! empty( $data ) ) {
				$value['settings']['order'] = $data;
				$update                     = true;
				pstoolkit_delete_option( 'ub_admin_bar_order' );
			}
			// Custom Admin Bar: custom css.
			$data = pstoolkit_get_option( 'ub_admin_bar_style' );
			if ( ! empty( $data ) ) {
				$value['css']['css'] = $data;
				$update              = true;
				pstoolkit_delete_option( 'ub_admin_bar_style' );
			}
			// Disable menu items.
			$data = pstoolkit_get_option( 'wdcab' );
			if ( ! empty( $data ) ) {
		...
		public function ajax_order_reset() {
			...
			$message = array(
				'type'    => 'success',
				'message' => sprintf( 'Die Reihenfolge der Admin-Leiste wurde zurückgesetzt.', 'ub' ),
			);
		...
		public function enqueue_scripts( $module ) {
			...
			// Module scripts.
			$file = pstoolkit_files_url( 'modules/admin/assets/js/jquery.classywiggle.min.js' );
			wp_enqueue_script( 'jquery-effects-wingle', $file, array( 'jquery' ), '1.2.0', true );

			if ( ! wp_script_is( 'pstoolkit-dashicon-select' ) ) {
				$dashicon_select = pstoolkit_files_url( 'modules/admin/assets/js/admin/dashicon-select.js' );
				wp_enqueue_script( 'pstoolkit-dashicon-select', $dashicon_select, array( 'jquery' ), $this->build, true );
			}

			if ( ! wp_style_is( 'pstoolkit-dashicon-select' ) ) {
				wp_enqueue_style(
					'pstoolkit-dashicon-select',
					pstoolkit_files_url( 'modules/admin/assets/css/admin/dashicon-select.css' ),
					array(),
					$this->build
				);
			}
		...
		public function get_admin_bar_menu_nodes( $wp_admin_bar ) {
			...
			if ( empty( $value ) ) {
				...
				$value['comments']    = __( 'Kommentare', 'ub' );
		...
		protected function set_options() {
			...
			$options = array(
				...
				'visibility' => array(
					'title'       => __( 'Toolbar Sichtbarkeit', 'ub' ),
					'description' => __( 'Standardmäßig ist die Symbolleiste nur für angemeldete Benutzer sichtbar. Du kannst die Sichtbarkeit jedoch auch für abgemeldete Benutzer ändern.', 'ub' ),
					'fields'      => array(
						'visibility'              => array(
							'type'        => 'checkboxes',
							'label'       => __( 'Front-End', 'ub' ),
							'description' => __( 'Wähle Benutzerrollen aus, um die Symbolleiste im Frontend anzuzeigen.', 'ub' ),
							'columns'     => 2,
							'options'     => $this->roles,
				...
				'items'      => array(
					'title'       => __( 'Menüelemente', 'ub' ),
					'description' => __( 'Passe das Menü an, indem Du Menüelemente basierend auf Benutzerrollen ausblendest, neu anordnest oder eigene benutzerdefinierte Menüelemente hinzufügst.', 'ub' ),
					'fields'      => array(
						'disabled_menus' => array(
							'type'         => 'checkboxes',
							'label'        => __( 'Menüpunkte', 'ub' ),
							'description'  => __( 'Wähle die Menüpunkte, die ausgeblendet werden sollen.', 'ub' ),
							'columns'      => 2,
						...
						'wp_menu_roles'  => array(
							'type'         => 'checkboxes',
							'label'        => __( 'Benutzerregeln', 'ub' ),
							'description'  => __( 'Wähle die Benutzerrollen aus, um die ausgewählten Menüelemente auszublenden.', 'ub' ),
							'columns'      => 2,
							'options'      => $this->roles,
							'master'       => $this->get_name( 'items-visibility' ),
							'master-value' => 'hide',
						...
						'visibility'     => array(
							...
							'description' => __( 'Wähle aus, ob Du alle Menüelemente anzeigen oder bestimmte Menüelemente vor bestimmten Benutzerrollen ausblenden möchtest.', 'ub' ),
							'options'     => array(
								'all'  => __( 'Zeige alles', 'ub' ),
								'hide' => __( 'Bestimmte Elemente ausblenden', 'ub' ),
							),
							'default'     => 'all',
						...
						'custom-entries' => array(
							'type'        => 'sui-tab',
							'label'       => __( 'Benutzerdefinierte Einträge', 'ub' ),
							'description' => __( 'Wähle aus ob Du benutzerdefinierte Einträge im Menü anzeigen möchtest oder nicht.', 'ub' ),
							'options'     => array(
						...
						'reorder'        => array(
							'label'       => __( 'Menüpunkte neu anordnen', 'ub' ),
							'description' => __( 'Klicke auf "Menüs neu anordnen" und ziehe die Menüelemente per Drag&Drop, um sie neu zu ordnen. "Standardreihenfolge wiederherstellen" setzt sie auf ihre ursprüngliche Reihenfolge zurück.', 'ub' ),
							'type'        => 'callback',
				...
				'css'        => array(
					...
					'fields'      => array(
						'css' => array(
							'type'          => 'css_editor',
							'placeholder'   => esc_html__( 'Gib hier benutzerdefiniertes CSS ein...', 'ub' ),
							'ace_selectors' => array(
								array(
									'title'     => '',
									'selectors' => array(
				...
				'builders'   => array(
					'title'       => __( 'Verbessere die Kompatibilität mit Page Buildern', 'ub' ),
					'description' => __( 'Wenn Du einen Page Builder verwendest und Kompatibilitätsprobleme mit der Admin-Leistenfunktion auftreten, füge die GET-Parameter für Deinen Page Builder hinzu, und PSToolkit überschreibt die Einstellungen auf diesen Seiten nicht.', 'ub' ),
					'fields'      => array(
						'get_params' => array(
							'type'        => 'textarea',
		...
		public function logo_output() {
			 ...
			 * @since 3.1.0
			 */
			$args = apply_filters( 'pstoolkit_admin_bar_logo_css_args', $args );
			/**
		...
		public function get_reorder() {
			...
			$args     = array(
				...
				'text' => __( 'Standardreihenfolge zurücksetzen', 'ub' ),
				'sui'  => 'ghost',
			);
			$content .= $this->button( $args );
			$content .= '</div>';
			...
			$args     = array(
				...
				'content' => __( 'Bist Du sicher, die Sonder-Sortierung zurückzusetzen?', 'ub' ),
				'footer'  => array(
					'content' => $footer,
					'classes' => array(
						'sui-actions-center',
		...
		public function get_list() {
			...
			$args     = array(
				...
				'icon'  => 'plus',
				'text'  => __( 'Benutzerdefiniertes Element hinzufügen', 'ub' ),
				'sui'   => 'magenta',
				'class' => 'pstoolkit-admin-bar-item-edit',
			);
			...
			$content .= sprintf(
				...
				empty( $items ) ? '' : ' pstoolkit-has-items'
			);
			...
			$content .= sprintf(
				...
				esc_html__( 'Es wurde noch kein benutzerdefinierter Menüpunkt hinzugefügt. Klicke auf "+Benutzerdefiniertes Element hinzufügen", um Dein erstes benutzerdefiniertes Menüelement mit einem einfachen Assistenten hinzuzufügen.', 'ub' )
			);
		...
		private function get_sui_tabs_config( $item = array() ) {
			$config = array(
				array(
					...
					'fields'   => array(
						'title'  => array(
							'label'       => __( 'Menütitel', 'ub' ),
							'value'       => isset( $item['title'] ) ? $item['title'] : '',
							'description' => array(
								'content'  => __( 'Du kannst auch die vollständige URL eines Bildes anstelle des Texttitels einfügen. Zum Beispiel http://example.com/img.png', 'ub' ),
								'position' => 'bottom',
							),
							'sui-row'     => 'begin',
							'required'    => 'required',
						...
						'icon'   => array(
							...
							'sui-row'     => 'end',
							'description' => array(
								'content'  => __( 'Wähle ein Symbol für Deinen benutzerdefinierten Menüpunkt.', 'ub' ),
								'position' => 'bottom',
							),
						...
						'url'    => array(
							'type'    => 'sui-tab',
							'label'   => __( 'Benutzer umleiten zu', 'ub' ),
							'options' => array(
								'none'     => __( 'Nirgends', 'ub' ),
								'main'     => __( 'Startseite', 'ub' ),
								'current'  => __( 'Aktuelle Seite', 'ub' ),
								'wp-admin' => __( 'WP Admin Bereich', 'ub' ),
								'custom'   => __( 'Benutzerdefinierte URL', 'ub' ),
							),
							'default' => 'none',
						...
						'custom' => array(
							'label'       => __( 'URL', 'ub' ),
							'placeholder' => esc_attr__( 'z.B. http://example.com', 'ub' ),
						...
						'target' => array(
							...
							'options' => array(
								'_blank' => __( 'neuer Registerkarte', 'ub' ),
								'_self'  => __( 'selbe Registerkarte', 'ub' ),
							),
							'default' => 'current',
				...
				array(
					...
					'fields'   => array(
						'items' => array(
							'description' => __( 'Ordne die Untermenüelemente durch Ziehen und Ablegen nach Bedarf neu an.', 'ub' ),
							'type'        => 'callback',
				...
				array(
					...
					'fields'   => array(
						'roles' => array(
							...
							'columns'     => 3,
							'label'       => __( 'Benutzerregeln', 'ub' ),
							'description' => __( 'Wähle die Benutzerrollen aus, die dieses Menü anzeigen dürfen.', 'ub' ),
							'options'     => $this->roles,
		...
		public function dashicons( $id = '{{data.id}}', $value = '' ) {
			...
			$args     = array(
				...
				'list'      => include pstoolkit_dir( '/etc/dashicons.php' ),
			);
			$content  = $this->render( $template, $args, true );
			return $content;
		}
		...
		private function get_sui_submenu_config( $item = array(), $id = '{{{data.id}}}' ) {
			$config = array(
				...
				'submenu][' . $id . '][target' => array(
					'type'    => 'sui-tab',
					'label'   => __( 'Link öffnen in', 'ub' ),
					'options' => array(
						'_blank' => __( 'neuer Registerkarte', 'ub' ),
						'_self'  => __( 'selben Registerkarte', 'ub' ),
					),
					'default' => 'current',
					'value'   => isset( $item['target'] ) ? $this->get_target( $item['target'] ) : '_self',
					'sui-row' => 'end',
				...
				'submenu][' . $id . '][url'    => array(
					...
					'label'   => '',
					'options' => array(
						'admin'  => __( 'Administrator Seite', 'ub' ),
						'site'   => __( 'Webseiten-Seite', 'ub' ),
						'custom' => __( 'Extern', 'ub' ),
					),
					'default' => 'none',
					'value'   => isset( $item['url'] ) ? $item['url'] : 'none',
					'content' => array(
						'admin'  => sprintf(
							'<label class="sui-label">%s</label><input type="text" aria-describedby="input-description" class="sui-form-control" placeholder="%s" name="pstoolkit[submenu][%s][url_admin]" value="%s" data-default="" /><p class="sui-description">%s</p>',
							esc_html__( 'URL', 'ub' ),
							esc_attr__( 'z.B. media.php', 'ub' ),
							esc_attr( $id ),
						...
						'site'   => sprintf(
							'<label class="sui-label">%s</label><input type="text" aria-describedby="input-description" class="sui-form-control" placeholder="%s" name="pstoolkit[submenu][%s][url_site]" value="%s" data-default="" />',
							esc_html__( 'URL', 'ub' ),
							esc_attr__( 'z.B. http://example.com', 'ub' ),
							esc_attr( $id ),
							isset( $item['url_site'] ) ? $item['url_site'] : ''
						),
						'custom' => sprintf(
							'<label class="sui-label">%s</label><input type="text" aria-describedby="input-description" class="sui-form-control" placeholder="%s" name="pstoolkit[submenu][%s][url_custom]" value="%s" data-default="" />',
							esc_html__( 'URL', 'ub' ),
							esc_attr__( 'z.B. http://example.com', 'ub' ),
							esc_attr( $id ),
		...
		public function submenu_items_list( $id ) {
			...
			$content .= '<div class="sui-accordion pstoolkit-sui-accordion-sortable">';
			if ( isset( $data['submenu'] ) && is_array( $data['submenu'] ) ) {
				foreach ( $data['submenu'] as $id => $item ) {
					...
					$content .= $this->button( $args );
					$content .= '<span class="pstoolkit-action-divider"></span>';
			...
			$content .= sprintf(
				...
				$this->button( $args ),
				esc_html__( 'Es wurde noch kein Untermenüelement hinzugefügt. Klicke auf "+Element hinzufügen", um ein Untermenüelement hinzuzufügen', 'ub' )
			);
			$content .= '</div>';
			$content .= '</div>';
			$content .= '</div>';
		...
		public function ajax_save_item() {
			...
			$data = $this->sanitize_request_payload( $_POST['pstoolkit'] );
			/**
			 * remove nonce
			 */
			if ( isset( $data['nonce'] ) ) {
		...
		public function ajax_delete_item() {
			...
			if ( isset( $items[ $id ] ) ) {
				...
				if ( $result ) {
					$message = array(
						'type'    => 'success',
						'message' => sprintf( 'Element wurde gelöscht.', 'ub' ),
					);
					$this->uba->add_message( $message );
					wp_send_json_success(
						array(
							'id'      => $id,
							'message' => __( 'Das ausgewählte Element wurde erfolgreich gelöscht', 'ub' ),
						)
					);
				}
			}
			// Send ajax response.
			wp_send_json_error( array( 'message' => __( 'Ausgewählter Artikel existiert nicht!', 'ub' ) ) );
		}
...
UBSTYLE;
		...
		public function add_dialog( $content, $module ) {
			...
			$content .= $this->get_dialog_delete(
				null,
				array(
					'title'       => __( 'Benutzerdefinierten Menüpunkt löschen', 'ub' ),
					'description' => __( 'Möchtest Du diesen benutzerdefinierten Menüpunkt wirklich dauerhaft löschen?', 'ub' ),
				)
			);
			/**
			 * Dialog settings
		...
		private function add_popular_builders_dialog() {
			...
			$add_args    = array(
				...
				'class'    => 'pstoolkit-admin-bar-add-popular-builders',
				'icon'     => 'check',
				'disabled' => true,
			);
			...
			$dialog_args = array(
				...
				'title'   => __( 'Builder GET-Parameter', 'ub' ),
				'classes' => array(
					'sui-modal-sm',
				),
			);
		...
		public function hide_toolbar_based_on_user_role( $show ) {
			...
			$show          = PSToolkit_Helper::is_allowed_role( $allowed_roles );
