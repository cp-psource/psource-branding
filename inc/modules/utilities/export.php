<?php
/**
 * PSToolkit Export class.
 *
 * @package PSToolkit
 * @subpackage Settings
 */
if ( ! class_exists( 'PSToolkit_Export' ) ) {

	class PSToolkit_Export extends PSToolkit_Helper {
		...
		public function __construct() {
			 ...
			 * hooks
			 */
			add_filter( 'pstoolkit_settings_export', array( $this, 'admin_options_page' ) );
			add_filter( 'pstoolkit_settings_export_process', array( $this, 'export' ) );
			/**
		...
		public function export() {
			...
			$options_names = apply_filters( 'pstoolkit_options_names', array() );
			/**
			 * get modules
			 */
			...
			if ( $is_subsite ) {
				...
				$uba           = pstoolkit_get_uba_object();
				$options_names = apply_filters( 'pstoolkit_subsites_options_names', array() );
				$modules       = apply_filters( 'pstoolkit_subsites_allowed_modules', $modules );
			}
			/**
			 * Remove some keys
			 */
			...
			$data = apply_filters( 'pstoolkit_export_data', $data );
			/**
			 * options to remove
			 */
			$remove = array(
			...
			foreach ( $options_names as $name ) {
				...
				$option_value = pstoolkit_get_option( $name );
				/**
		...
		protected function set_options() {
			$this->options = array(
				'export' => array(
					'title'       => __( 'Exportieren', 'ub' ),
					'description' => __( 'Exportiere Deine PSToolkit-Konfigurationen in eine JSON-Datei, um sie auf einer anderen Webseite zu verwenden.', 'ub' ),
					'fields'      => array(
						'add_debug_information' => array(
							'type'           => 'checkbox',
							'checkbox_label' => __( 'Debug-Informationen einschließen', 'ub' ),
							'description'    => array(
								'content'  => __( 'Wenn Du dies wählst, erhältst Du Debug-Informationen zu den installierten Designs und Plugins in der Exportdatei.', 'ub' ),
								'position' => 'bottom',
							),
							'group'          => array(
								'begin'   => true,
