<?php
/**
 * PSToolkit Administrator Menu class.
 *
 ...
 */
if ( ! class_exists( 'PSToolkit_Admin_Menu' ) ) {

	/**
	 * Class PSToolkit_Admin_Menu.
	 */
	class PSToolkit_Admin_Menu extends PSToolkit_Helper {
		...
		/**
		 * PSToolkit_Admin_Menu constructor.
		 */
		public function __construct() {
			parent::__construct();
			$this->module = 'admin_menu';
			// Base hooks.
			add_filter( 'pstoolkit_settings_admin_menu', array( $this, 'admin_options_page' ) );
			add_filter( 'pstoolkit_settings_admin_menu_process', array( $this, 'update' ) );
			...
			add_action( 'wp_ajax_pstoolkit_admin_bar_load_menu', array( $this, 'ajax_load_menu' ) );
			add_action( 'wp_ajax_pstoolkit_admin_bar_save_menu', array( $this, 'ajax_save_menu' ) );
			add_action( 'wp_ajax_pstoolkit_admin_bar_remove_menu', array( $this, 'ajax_delete_menu' ) );
			add_action( 'wp_ajax_pstoolkit_admin_menu_search_user', array( $this, 'ajax_search_user' ) );
		...
		public function enqueue_global_admin_assets() {
			wp_enqueue_style(
				'pstoolkit-admin-menu-global',
				pstoolkit_files_url( 'modules/admin/assets/css/admin/admin-menu-global.css' ),
				array(),
				$this->build
			);
		}
		...
		public function end_admin_menu_buffer() {
			...
			if ( ! class_exists( 'PSToolkit_Admin_Menu_Builder' ) ) {
				require_once dirname( __FILE__ ) . '/custom-admin-menu/pstoolkit-admin-menu-builder.php';
			}
			global $self, $parent_file, $submenu_file, $plugin_page, $typenow;

			$builder = new PSToolkit_Admin_Menu_Builder(
				$menu,
				$self,
				$parent_file,
				$submenu_file,
		...
		public function block_hidden_page() {
			if ( wp_doing_ajax() || ! apply_filters( 'pstoolkit_custom_admin_menu_block_access', true ) ) {
				return;
			}
			...
			foreach ( $menu as $menu_item_index => $menu_item ) {
				...
				// Disable option
				$is_menu_item_hidden = (bool) pstoolkit_get_array_value(
					$menu_settings,
					array(
						$menu_item_id,
						'is_hidden',
				...
				// Hide option
				$is_menu_item_invisible = (bool) pstoolkit_get_array_value(
					$menu_settings,
					array(
						$menu_item_id,
						'is_invisible',
				...
				if ( ! empty( $submenu[ $menu_slug ] ) ) {
					foreach ( $submenu[ $menu_slug ] as $submenu_item_index => $submenu_item ) {
						...
						// Disable option
						$is_submenu_item_hidden = (bool) pstoolkit_get_array_value(
							$menu_settings,
							array(
								$menu_item_id,
								'submenu',
						...
						// Hide option
						$is_submenu_item_invisible = (bool) pstoolkit_get_array_value(
							$menu_settings,
							array(
								$menu_item_id,
								'submenu',
		...
		private function prevent_admin_page_access( $slug ) {
			...
			$uri_path = pstoolkit_get_array_value( $uri, 'path' );
			if ( empty( $uri_path ) ) {
				return;
			}
			$uri_query = pstoolkit_get_array_value( $uri, 'query' );
			...
			$is_plugin_page = urldecode( $uri_query ) === "page=$slug"; // e.g. /wp-admin/admin.php?page=pstoolkit
			if ( $is_plugin_page ) {
		...
		public function show_admin_page_access_error() {
			wp_die( __( 'Leider darfst Du nicht auf diese Seite zugreifen.', 'ub' ), 403 );
		}
		...
		public function enqueue_scripts( $module ) {
			...
			if ( ! wp_script_is( 'pstoolkit-dashicon-picker' ) ) {
				$dashicon_select = pstoolkit_files_url( 'modules/admin/assets/js/admin/dashicon-picker.js' );
				wp_enqueue_script( 'pstoolkit-dashicon-picker', $dashicon_select, array( 'jquery' ), $this->build, true );
			}

			if ( ! wp_style_is( 'pstoolkit-dashicon-picker' ) ) {
				wp_enqueue_style(
					'pstoolkit-dashicon-picker',
					pstoolkit_files_url( 'modules/admin/assets/css/admin/dashicon-picker.css' ),
					array(),
					$this->build
				);
			}
		...
		function enqueue_menu_assets() {
			...
			$base_url = $admin_menu = pstoolkit_files_url( 'modules/admin/assets/js/admin/admin-menu/' );

			foreach ( $scripts as $script ) {
				$script_id = 'pstoolkit-' . $script;
			...
			wp_localize_script(
				'pstoolkit-custom-admin-menu',
				'pstoolkit_custom_menu',
				array(
					'discard_confirm'     => esc_html__( 'Möchtest Du wirklich alle Änderungen verwerfen?', 'ub' ),
					'duplicate_postfix'   => esc_html_x( '[DUPLIKAT]', 'Postfix für doppelte Elemente - benutzerdefiniertes Administratormenü', 'ub' ),
					'nonce'               => wp_create_nonce( 'pstoolkit-custom-admin-menu' ),
					'user_id'             => get_current_user_id(),
					'user_menu_keys'      => $this->get_user_menu_keys(),
					'role_menu_keys'      => $this->get_subordinates_roles(),
					'setting_updated'     => esc_html__( 'Deine Einstellungen wurden aktualisiert!', 'ub' ),
					'title_missing_error' => esc_html__( 'Benenne das Element um fortzufahren.', 'ub' ),
				)
			);
		}
		...
		public function add_dialog( $content, $module ) {
			...
			$no_menu_items                = $pstoolkit_network && is_network_admin() && ! $saved_menu;
		...
		protected function set_options() {
			$options       = array(
				'custom-admin-menu' => array(
					'title'       => __( 'Benutzerdefiniertes Admin-Menü', 'ub' ),
					'description' => __( 'Passe das Admin-Menü nach Benutzerrollen oder nach benutzerdefiniertem Benutzer an. Du kannst die Menüelemente nach Bedarf hinzufügen, ausblenden und neu anordnen.', 'ub' ),
					'fields'      => array(
						'button' => array(
							'type'  => 'button',
							'value' => __( 'Anpassen', 'ub' ),
							'icon'  => 'wrench-tool',
							'data'  => array(
				...
				'dashboard-link'    => array(
					...
					'description'  => __( 'Entferne den Link "Dashboard" aus dem Admin-Bereich für Benutzer ohne Webseite (in WP Multisite).', 'ub' ),
					'fields'       => array(
						'status' => array(
							'checkbox_label' => __( 'Link für Benutzer ohne Webseite entfernen', 'ub' ),
							'type'           => 'checkbox',
				...
				'tips'              => array(
					'title'       => __( 'Admin-Tipps', 'ub' ),
					'description' => __( 'Stelle Deinen Benutzern hilfreiche zufällige Tipps oder Werbeaktionen/Nachrichten in ihren Admin-Panels zur Verfügung.', 'ub' ),
					'fields'      => array(
						'status' => array(
		...
		/**
		 * Menüelemente, die den Benutzer zu customizer.php führen, können eine Abfragevariable 'return' enthalten..
		 * Der Wert dieser Variable hängt von der Seite ab, auf der sich der Benutzer gerade befindet.
		 * Da wir href von Menüelementen als Bezeichner verwenden, müssen wir diesen variablen Teil entfernen.
		 *
		 * @param $slug
		 *
		 * @return string
		...
		private function build_menu_item_settings_array() {
			return array(
				'title'         => array(
					'name'        => 'title',
					'type'        => 'text',
					'label'       => esc_html__( 'Titel des Menüelements', 'ub' ),
					'description' => array(
						'content'  => esc_html__( 'Gib diesem Menüpunkt einen Titel.', 'ub' ),
						'position' => 'bottom',
					),
					'default'     => '{{ data.title }}',
				...
				'id_attribute'  => array(
					'name'         => 'id_attribute',
					'type'         => 'text',
					'label'        => esc_html__( 'ID-Attribut', 'ub' ),
					'description'  => array(
						'content'  => esc_html__( 'Muss einzigartig sein.', 'ub' ),
						'position' => 'bottom',
					),
				...
				'css_classes'   => array(
					'name'         => 'css_classes',
					'type'         => 'text',
					'label'        => esc_html__( 'CSS-Klassen', 'ub' ),
					'description'  => array(
						'content'  => esc_html__( 'Füge Deine eigene CSS-Klasse hinzu. Trenne mehrere Klassen durch ein Leerzeichen.', 'ub' ),
						'position' => 'bottom',
					),
				...
				'icon_url'      => array(
					...
					'display'      => 'sui-tab-content',
					'description'  => array(
						'content'  => __( 'Gib die URL Deines Symbols ein. Dies sollte vorzugsweise eine PNG-Datei sein.', 'ub' ),
						'position' => 'bottom',
					),
				...
				'icon_image_id' => array(
					...
					'display'      => 'sui-tab-content',
					'description'  => array(
						'content'  => sprintf( __( 'Lade ein Symbol hoch, um das Standardmenüelementsymbol zu überschreiben. Die empfohlene Größe ist %s.', 'ub' ), '<strong>20x20</strong>' ),
						'position' => 'bottom',
					),
				),
				'dashicon'      => array(
				...
				'link_type'     => array(
					...
					'options'     => array(
						'none'     => __( 'Keinen', 'ub' ),
						'frontend' => __( 'Startseite', 'ub' ),
						'admin'    => __( 'WP Admin Bereich', 'ub' ),
		...
		public function print_icon_svg_textarea() {
			...
			<span class="sui-description">
				<?php esc_html_e( "Füge eine Base64-codierte SVG-Datei mit einem Daten-URI ein, der entsprechend dem Farbschema farbig dargestellt wird. Dies sollte mit 'data: image / svg + xml; base64' beginnen.", 'ub' ); ?>
			</span>
			<?php
			return ob_get_clean();
		}
		...
		public function print_icon_uploader() {
			return '<div class="pstoolkit-admin-menu-icon-uploader"></div>';
		}
		...
		public function ajax_load_menu() {
			check_ajax_referer( 'pstoolkit-custom-admin-menu' );

			if ( ! PSToolkit_Permissions::get_instance()->current_user_has_access() ) {
				wp_send_json_error( array( 'message' => esc_html__( 'Zugriff verweigert', 'ub' ) ) );
			}

			$data     = stripslashes_deep( $_POST );
			$menu_key = sanitize_key( pstoolkit_get_array_value( $data, 'key' ) );
			if ( empty( $menu_key ) ) {
				wp_send_json_error( array( 'message' => esc_html__( 'Etwas ist schief gelaufen', 'ub' ) ) );
			}
			...
			do_action( 'pstoolkit_custom_admin_menu_loaded', $menu );
		...
		/**
		 * Remove old 3party plugin's menu which were set in PSToolkit settings but deactivated right now
		 *
		 * @param array $menu
		 * @return array
		 */
		private function remove_old_native_items( $menu ) {
			foreach ( $menu as $menu_item_id => $menu_item_settings ) {
				$is_native  = (bool) pstoolkit_get_array_value( $menu_item_settings, 'is_native' );
				$was_native = (bool) pstoolkit_get_array_value( $menu_item_settings, 'was_native' );
				...
				$submenu = pstoolkit_get_array_value( $menu_item_settings, 'submenu' );
				if ( is_array( $submenu ) ) {
			...
			foreach ( $defaults as $default_id => $default_value ) {
				if ( is_array( $default_value ) ) {
					$parsed_value = $this->parse_args_deep(
						pstoolkit_get_array_value( $array, $default_id ),
						$default_value
					);
		...
		private function menu_item_extras( $title_default = '', $slug = '', $url_default = '', $id_attribute_default = '', $css_classes_default = '', $icon_default = '' ) {
			...
			$extras = array(
				// A native item is one that is not created through PSToolkit
				'is_native'            => true,
		...
		public function ajax_save_menu() {
			...
			$data      = stripslashes_deep( $_POST );
			$menu_key  = sanitize_key( pstoolkit_get_array_value( $data, 'key' ) );
			$menu_json = pstoolkit_get_array_value( $data, 'menu' );
			...
			if ( empty( $menu ) ) {
				wp_send_json_error( array( 'message' => esc_html__( 'Menüdaten konnten nicht analysiert werden', 'ub' ) ) );
				return;
			}
		...
		public function maybe_load_tips() {
			...
			$file = pstoolkit_files_dir( 'modules/admin/tips.php' );
			include_once $file;
		}
		...
		private function replace_users_menu_item( $menu, $submenu ) {
			foreach ( $menu as $menu_item_priority => $menu_item ) {
				if ( pstoolkit_get_array_value( $menu_item, 2 ) === 'users.php' ) {
					array_splice(
						$menu_item,
						0,
						3,
						array( __( 'Profil', 'ub' ), 'read', 'profile.php' )
					);
			...
			if ( isset( $submenu['users.php'] ) && is_array( $submenu['users.php'] ) ) {
				foreach ( $submenu['users.php'] as $submenu_item_priority => $submenu_item ) {
					if ( pstoolkit_get_array_value( $submenu_item, 2 ) !== 'profile.php' ) {
		...
		public function save_all_menus( $parent_file ) {
			global $menu, $submenu, $pstoolkit_network;
			if (
				! is_admin()
				|| ! PSToolkit_Permissions::get_instance()->current_user_has_access()
				|| ( $pstoolkit_network && is_network_admin() )
			) {
				return $parent_file;
			}
		...
		private function get_menu_settings( $key = '', $default = array() ) {
			$menus = pstoolkit_get_option( self::CUSTOM_ADMIN_MENU_OPTION );
			if ( empty( $key ) ) {
			...
			$menu = pstoolkit_get_array_value( $menus, $key );
		...
		private function set_menu_settings( $key, $menu_settings ) {
			...
			return pstoolkit_update_option( self::CUSTOM_ADMIN_MENU_OPTION, $menus );
		}
