<?php
/**
 * PSToolkit Signup Password class.
 *
 * @package PSToolkit
 * @subpackage Front-end
 */
if ( ! class_exists( 'PSToolkit_Signup_Password' ) ) {

	class PSToolkit_Signup_Password extends PSToolkit_Helper {
		...
		public function password_filter( $content ) {
			...
			if ( ! empty( $password_1 ) && 'validate-user-signup' === $_POST['stage'] ) {
				if ( $password_1 != $password_2 ) {
					$content['errors']->add( 'password_1', __( 'Passwörter stimmen nicht überein.', 'ub' ) );
				}
			}
			return $content;
		}
...
Dein neues Konto ist eingerichtet.

Du kannst Dich mit den folgenden Informationen anmelden:
...
Deines neues Konto ist eingerichtet.
...
--Das Team @ SITE_NAME',
		...
		public function enqueue_style() {
			global $ub_version;
			$file = pstoolkit_files_url( 'modules/login-screen/assets/css/signup-password.css' );
