(function ($) {
	...
	/**
	 * @param id
	 * @param model {PSToolkit.Menu_Item_Model}
	 * @constructor
	 */
	var Menu_Item_Elements = function (id, model) {
		...
		this.root = $(markup);
		this.submenu_container = $('.pstoolkit-submenu-container', this.root);
		this.item_header = $('> .sui-accordion-item-header', this.root);
		this.item_body = $('> .sui-accordion-item-body', this.root);

		this.duplicate_button = $('.pstoolkit-custom-admin-menu-duplicate', this.item_header);
		this.is_invisible_checkbox = $('.pstoolkit-custom-admin-menu-is-invisible input[type=checkbox]', this.item_header);
		this.is_hidden_checkbox = $('.pstoolkit-custom-admin-menu-is-hidden input[type=checkbox]', this.item_header);
		this.is_selected_checkbox = $('.pstoolkit-custom-admin-menu-is-selected', this.item_header);
		this.icon_container = $('.pstoolkit-menu-item-icon-container', this.item_header);
		this.item_header_title = $('.pstoolkit-menu-item-title', this.item_header);
		this.remove_button = $('.pstoolkit-custom-admin-menu-remove', this.item_header);
		this.undo_button = $('.pstoolkit-custom-admin-menu-undo', this.item_header);
		this.item_controls = $('.pstoolkit-custom-admin-menu-controls', this.item_header);
		this.dropdown_anchor = $('.sui-dropdown-anchor', this.item_controls);

		this.settings_container = $('.pstoolkit-menu-item-settings-container', this.item_body);
		this.title_form_field = $('[name="title"]', this.item_body).closest('.sui-form-field');
	};

	window.PSToolkit.Menu_Item = function (id, item, container) {
		// Public methods
		...
		var menu_item_id = id;
		/**
		 * @type {PSToolkit.Menu_Item_Model}
		 */
		var model = new PSToolkit.Menu_Item_Model(item);
		/**
		 * @type {PSToolkit.Menu_Item}
		 */
		var self = this;
		/**
		 * @type {PSToolkit.Menu_Icon_Uploader}
		 */
		var uploader;
		...
		function init_dashicon_select() {
			...
			new PSToolkit.Dashicon_Picker($input, value);
		...
		function init_icon_uploader() {
			uploader = new PSToolkit.Menu_Icon_Uploader(
				$('.pstoolkit-admin-menu-icon-uploader', elements.settings_container),
		...
		function build_masked_icon_el(mask_image) {
			...
			return $('<div></div>')
				.addClass('pstoolkit-custom-admin-menu-mask')
		...
		function build_image_icon_el(url) {
			...
			return $('<div></div>')
				.addClass('pstoolkit-custom-admin-menu-img')
				.css('background-image', value);
		}

		function init_submenu() {
			if (is_main() && !submenu) {
				submenu = new PSToolkit.Menu_Item_Container();
				...
				for (var key in submenu_data) {
					...
					submenu.add(new PSToolkit.Menu_Item(key, submenu_params, submenu));
				}
		...
		function replicate_data() {
			...
			var replicated = $.extend({}, model.get_values(), {
				is_native: false,
				was_native: false,
				title: model.get_title() + ' ' + pstoolkit_custom_menu.duplicate_postfix
			});
		...
		function make_invisible() {
			elements.is_invisible_checkbox.each( function() {
				...
				if ( ! $parent.hasClass( 'pstoolkit-menu-item-hidden' ) ) {
		...
		function hide() {
			elements.is_hidden_checkbox.each( function() {
				...
				if ( ! $parent.hasClass( 'pstoolkit-menu-item-invisible' ) ) {
					$(this).prop('checked', true).trigger('change');
				}
			});
		}
		...
		/**
		 * @returns {PSToolkit.Menu_Item}
		 */
		function create_duplicate_object() {
			var params = replicate_data(),
				duplicate_id = PSToolkit.Menu_Utils.generate_random_menu_item_id();

			return new PSToolkit.Menu_Item(duplicate_id, params, container);
		}
		...
		function init_is_invisible_checkbox() {
			elements.is_invisible_checkbox.on('change', function () {
				...
				$accordion_item
					...
					.toggleClass('pstoolkit-menu-item-invisible', is_checked);
		...
		function init_is_hidden_checkbox() {
			elements.is_hidden_checkbox.on('change', function () {
				...
				$accordion_item
					.removeClass(accordion_open_class())
					.toggleClass('pstoolkit-menu-item-hidden', is_checked);
		...
		function validate() {
			var title = model.get_title() + '',
				error = '<span class="hidden sui-error-message">' + pstoolkit_custom_menu.title_missing_error + '</span>',
		...
		function init_undo_button() {
			var dirty_class = 'pstoolkit-menu-item-dirty';
		...
		function undo() {
			var fresh = new PSToolkit.Menu_Item(get_id(), item, container);
		...
		function init_controls_dropdown() {
			...
			PSToolkit.Window_Resize_Manager.on_resize(maybe_show_controls_as_dropdown);
