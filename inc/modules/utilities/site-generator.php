<?php
/**
 * PSToolkit Site Generator class.
 *
 * @package PSToolkit
 * @subpackage Utilites
 */
if ( ! class_exists( 'PSToolkit_Site_Generator' ) ) {

	class PSToolkit_Site_Generator extends PSToolkit_Helper {
		...
		public function __construct() {
			...
			add_filter( 'pstoolkit_settings_site_generator', array( $this, 'admin_options_page' ) );
			add_filter( 'pstoolkit_settings_site_generator_process', array( $this, 'update' ) );
			/**
			 * replace
			 */
			...
			if ( $this->check_defender ) {
				add_filter( 'pstoolkit_settings_tab_content_classes', array( $this, 'add_class_if_defender' ), 10, 2 );
			}
		}
		...
		protected function set_options() {
			$options = array(
				'generator' => array(
					'title'       => __( 'Seiten Generator Info', 'ub' ),
					'description' => __( 'Ändere die Standardgeneratorinformationen, die in der Seitenquelle Deiner Webseite angezeigt werden.', 'ub' ),
					'fields'      => array(
						'text'    => array(
							'label'       => __( 'Text', 'ub' ),
			...
			if ( $this->check_defender ) {
				...
				if ( $defender ) {
					$options['generator']['classes'] = array( 'pstoolkit-not-affected' );
					/**
					 * get template
					 */
		...
		public function upgrade_options() {
			$value = pstoolkit_get_option( 'site_generator_replacement' );
			if ( ! empty( $value ) ) {
				$this->set_value( 'generator', 'text', $value );
				pstoolkit_delete_option( 'site_generator_replacement' );
			}
			$value = pstoolkit_get_option( 'site_generator_replacement_link' );
			if ( ! empty( $value ) ) {
				$this->set_value( 'generator', 'link', $value );
				pstoolkit_delete_option( 'site_generator_replacement_link' );
			}
		}
		...
		/**
		 * Add class to pstoolkit-settings-tab-content
		 *
		 * @since 1.0.0
		 *
