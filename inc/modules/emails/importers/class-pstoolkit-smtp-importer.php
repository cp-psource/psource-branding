...
abstract class PSToolkit_SMTP_Importer {
	protected $option;
	protected $translate;
	protected $module;
	...
	protected function proceed() {
		$value = pstoolkit_get_option( $this->option );
		...
		foreach ( $this->translate as $group_key => $group_data ) {
			foreach ( $group_data as $field_key => $imported_key ) {
				...
				if (
					...
					$filter = sprintf( 'pstoolkit_smtp_import_%s_%s_%s', $this->option, $imported_key[0], $imported_key[1] );
				...
				if (
					...
					$filter = sprintf( 'pstoolkit_smtp_import_%s_%s', $this->option, $imported_key );
