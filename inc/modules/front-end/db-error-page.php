...
if ( ! class_exists( 'PSToolkit_DB_Error_Page' ) ) {

	/**
	 * Class PSToolkit_DB_Error_Page
	 */
	class PSToolkit_DB_Error_Page extends PSToolkit_Helper {
		...
		/**
		 * PSToolkit_DB_Error_Page constructor.
		 */
		public function __construct() {
			...
			add_filter( 'pstoolkit_settings_db_error_page', array( $this, 'admin_options_page' ) );
			// If module ready, register hooks.
			if ( $this->is_ready ) {
				add_filter( 'pstoolkit_settings_db_error_page_process', array( $this, 'update' ) );
			}
			...
			add_filter( 'pstoolkit_settings_db_error_page_preserve', array( $this, 'add_preserve_fields' ) );
		}

		public function set_default_value( $data, $module, $section, $name ) {
			if ( $this->module === $module && false === $data && empty( $section ) && empty( $name ) ) {
				pstoolkit_update_option( $this->option_name, array() );
			}
		...
		private function update_file( $value ) {
			...
			$body_classes = array( 'pstoolkit-settings-db-error-page' );
			/**
			 * Title
			 */
			$title = $this->get_value( 'content', 'content_title', null );
			if ( empty( $title ) ) {
				$title = __( 'Wir werden bald zurück sein!', 'ub' );
			}
			/**
			 * Content
			 */
			...
			if ( empty( $content ) ) {
				$content = wpautop( __( 'Wir haben derzeit technische Probleme &mdash; Bitte schaue bald wieder vorbei...', 'ub' ) );
			}
			...
			if ( 'on' === $send_email ) {
				...
				if ( ! empty( $email ) ) {
					$mail_content  = esc_html__( 'Es liegt ein Problem mit der Datenbank vor!', 'ub' );
					$mail_content .= PHP_EOL;
					$mail_content .= PHP_EOL;
					$mail_content .= sprintf( _x( 'Seiten URL: %s', 'DB-Fehlermodul: mail', 'ub' ), home_url() );
					$mail_content .= PHP_EOL;
					$mail_content .= sprintf( _x( 'Seiten Name: %s', 'DB-Fehlermodul: mail', 'ub' ), get_bloginfo( 'name' ) );
					$php          .= sprintf(
						"mail('%s', '%s', '%s', 'Von: \"%s\" <%s>');",
						esc_attr( $email ),
						esc_html__( 'Datenbankfehler', 'ub' ),
						$mail_content,
		...
		protected function set_options() {
			if ( ! $this->is_ready ) {
				...
				if ( false == $this->is_ready_dir ) {
					$value = sprintf(
						__( 'Das Verzeichnis %s ist nicht beschreibbar. Wir können keine Datei db-error.php erstellen.', 'ub' ),
						sprintf( '<code>%s</code>', $this->db_error_dir )
					);
				} elseif ( false === $this->is_ready_file ) {
					$value = sprintf(
						__( 'Datei %s ist nicht beschreibbar, wir können sie nicht ändern.', 'ub' ),
					...
					'settings' => array(
						'fields' => array(
							'message' => array(
								'hide-th' => true,
								'type'    => 'description',
								'value'   => PSToolkit_Helper::sui_notice( $value ),
							),
						),
					),
				);
			...
			$options = array(
				...
				'content' => array(
					'title'       => __( 'Inhalt', 'ub' ),
					'description' => __( 'Wähle das Verhalten, wenn ein Besucher beim Surfen auf Deiner Webseite auf einen DB-Fehler stößt.', 'ub' ),
					'show-as'     => 'accordion',
				...
				'design'  => array(
					'title'       => __( 'Design', 'ub' ),
					'description' => __( 'Passe das Design jedes Elements Deines DB-Fehlerbildschirms an.', 'ub' ),
					'show-as'     => 'accordion',
					'fields'      => $this->get_options_fields(
						'design',
				...
				'mail'    => array(
					...
					'fields'      => array(
						...
						'send' => array(
							'type'        => 'sui-tab',
							'label'       => __( 'Benachrichtigungs-E-Mail senden', 'ub' ),
							'description' => __( 'Sende eine Benachrichtigungs-E-Mail bezüglich des Auftretens eines Datenbankfehlers auf der Webseite.', 'ub' ),
							'options'     => array(
			...
			if ( ! is_file( $file ) || ! is_readable( $file ) ) {
				$options['preview']['fields']['preview'] = array(
					'type'  => 'description',
					'value' => PSToolkit_Helper::sui_notice( __( 'Vorschau ist nicht verfügbar. Speichere zuerst die Einstellungen!', 'ub' ), 'info' ),
				);
			}
			$this->options = $options;
		}
		...
		public function get_options_fields_design_error_message( $defaults = array() ) {
			$data = array(
				'text_aligment' => array(
					'type'      => 'sui-tab-icon',
					'label'     => __( 'Titeltextausrichtung', 'ub' ),
					'options'   => array(
						'left'   => 'align-left',
						'center' => 'align-center',
						'right'  => 'align-right',
