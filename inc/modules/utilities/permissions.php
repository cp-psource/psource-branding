...
if ( ! class_exists( 'PSToolkit_Permissions' ) ) {
	...
	/**
	 * Class PSToolkit_Permissions.
	 */
	class PSToolkit_Permissions extends PSToolkit_Helper {
		...
		/**
		 * Return the PSToolkit_Permissions instance
		 *
		 * @since 3.2
		 * @return PSToolkit_Permissions
		 */
		...
		/**
		 * PSToolkit_Permissions constructor.
		 *
		 * @since 3.2.0
		 */
		public function __construct() {
			...
			add_filter( 'pstoolkit_settings_permissions', array( $this, 'admin_options_page' ) );
			add_filter( 'pstoolkit_settings_permissions_process', array( $this, 'update' ) );
			...
			add_filter( 'pstoolkit_module_check_for_subsite', array( $this, 'check_module' ), 10, 3 );
			add_filter( 'pstoolkit_group_check_for_subsite', array( $this, 'check_group' ), 10, 3 );
			add_filter( 'pstoolkit_force_local_option', array( $this, 'check_option' ), 10, 2 );
			/**
			 * pstoolkit subsites options name
			 */
			add_filter( 'pstoolkit_subsites_options_names', array( $this, 'get_options_names' ) );
			/**
			 * pstoolkit subsites allowed modules
			 */
			add_filter( 'pstoolkit_subsites_allowed_modules', array( $this, 'get_allowed_modules' ) );
			/**
			 * Add dialog
			 *
			 * @since 3.2
			 ...
			 * AJAX
			 */
			add_action( 'wp_ajax_pstoolkit_usersearch', array( $this, 'usersearch' ) );
			add_action( 'wp_ajax_pstoolkit_permissions_add_user', array( $this, 'ajax_save_user' ) );
			add_action( 'wp_ajax_pstoolkit_permissions_delete_user', array( $this, 'ajax_delete_user' ) );
			add_action( 'wp_ajax_pstoolkit_notice_permissions_notice_save', array( $this, 'ajax_save_subsite_messsage_status' ) );
			add_filter( 'pstoolkit_subsites_dashboard_message', array( $this, 'add_dashboard_message_data' ) );
		}
		...
		protected function set_options() {
			...
			$options = array(
				'permissions' => array(
					'title'       => __( 'Nutzerberechtigung', 'ub' ),
					'description' => __( 'Konfiguriere welche Benutzer und Benutzerrollen auf die Einstellungen von PSToolkit zugreifen und diese konfigurieren können.', 'ub' ),
					'fields'      => array(
						'user_roles' => array(
							'label'           => __( 'Benutzerregeln', 'ub' ),
							'description'     => __( 'Wähle aus welche Benutzerrollen Zugriff haben können, und konfiguriere PSToolkit.', 'ub' ),
							'type'            => 'checkboxes',
							'columns'         => 2,
							'disabled'        => $pstoolkit_network ? array( 'super' ) : array( 'administrator' ),
							'always_selected' => $pstoolkit_network ? array( 'super' ) : array( 'administrator' ),
							'options'         => $this->roles,
						),
						'users'      => array(
							'type'        => 'callback',
							'callback'    => array( $this, 'get_users' ),
							'label'       => __( 'Benutzerdefinierte Benutzer', 'ub' ),
							'description' => __( 'Wähle zusätzlich zu den oben genannten Rollen bestimmte Benutzer aus, die auf PSToolkit zugreifen und dieses konfigurieren dürfen.', 'ub' ),
						),
					),
				),
			);
			...
			if ( $pstoolkit_network ) {
				$options['subsites'] = array(
					'title'       => __( 'Subseite-Steuerelemente', 'ub' ),
					'description' => __( 'Standardmäßig erben Unterwebsites Deine Netzwerkeinstellungen. Wähle aus, welche Module Subseiten-Administratoren überschreiben können.', 'ub' ),
					'fields'      => array(
						'items'  => array(
							'type'         => 'callback',
						...
						'none'   => array(
							'type'         => 'description',
							'value'        => PSToolkit_Helper::sui_notice( esc_html__( 'Administratoren von Unterwebseiten können keine Moduleinstellungen überschreiben und erben immer Deine Netzwerkeinstellungen.', 'ub' ), 'info' ),
							'master'       => $this->get_name( 'status' ),
						...
						'all'    => array(
							'type'         => 'description',
							'value'        => PSToolkit_Helper::sui_notice( esc_html__( 'Administratoren von Unterwebsites können alle in Deinen Netzwerkeinstellungen festgelegten Standardmoduleinstellungen überschreiben.', 'ub' ), 'info' ),
							'master'       => $this->get_name( 'status' ),
						...
						'status' => array(
							'type'        => 'sui-tab',
							'options'     => array(
								'disabled' => __( 'Keiner', 'ub' ),
								'enabled'  => __( 'Alle', 'ub' ),
								'modules'  => __( 'Benutzerdefiniert', 'ub' ),
							),
							'default'     => 'disabled',
			...
			$args = array(
				'button'      => $this->button(
					array(
						...
						'icon' => 'plus',
						'text' => __( 'Benutzer hinzufügen', 'ub' ),
						'sui'  => 'magenta',
					)
				),
		...
		public function ajax_save_user() {
			...
			$item['message'] = __( 'Benutzerberechtigungen erfolgreich aktualisiert.', 'ub' );
			/**
			 * Send it back
			 */
		...
		public function ajax_delete_user() {
			...
			if ( get_current_user_id() === $id ) {
				wp_send_json_error( array( 'message' => __( 'Du kannst Deinen Zugang nicht selbst entfernen!', 'ub' ) ) );
			}
			...
			if ( false !== $key ) {
				...
				wp_send_json_success(
					array(
						'id'      => $id,
						'message' => __( 'Benutzerberechtigungen erfolgreich aktualisiert.', 'ub' ),
					)
				);
			}
			wp_send_json_error( array( 'message' => __( 'Ausgewählter Benutzer existiert nicht!', 'ub' ) ) );
		}
		...
		private function update_allowed_users( $users = null ) {
			...
			pstoolkit_update_option( $this->option_name_users, $users );
		...
		public function current_user_has_access() {
			...
			$allow         = PSToolkit_Helper::is_allowed_role( $allowed_roles );
		...
		public function get_allowed_user_ids() {
			$permission_settings = pstoolkit_get_option_filtered( $this->option_name_users );
		...
		public function get_allowed_roles() {
			...
			/**
			 * Allowed roles who have access to PSToolkit settings
			 *
			 * @since 3.3
			 * @param array $roles WP roles
			 */
			return apply_filters( 'pstoolkit_permissions_allowed_roles', $roles );
		}
