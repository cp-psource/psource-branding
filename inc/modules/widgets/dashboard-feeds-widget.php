<?php
/**
 * PSToolkit Dashboard Feeds Widget class.
 *
 * @package PSToolkit
 * @subpackage Widgets
 */
if ( ! class_exists( 'PSToolkit_Dashboard_Feeds_Widget' ) ) {

	class PSToolkit_Dashboard_Feeds_Widget {
		var $widget_id;
		var $widget_options;
		...
		public function wp_dashboard_widget_display() {
			...
			if ( is_wp_error( $rss ) ) {
				if ( is_admin() || current_user_can( 'manage_options' ) ) {
					...
					printf( __( '<strong>Feed Fehler</strong>: %s', 'ub' ), $rss->get_error_message() );
					echo '</p></div>';
				}
