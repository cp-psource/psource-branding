<?php
if ( ! class_exists( 'PSToolkit_Email_Logs_CPT' ) ) {
	/**
	 * PSToolkit Email Logs post type class.
	 *
	 * @package PSToolkit
	 * @subpackage Emails
	 */
	class PSToolkit_Email_Logs_CPT {
		/**
		 * Instance
		 *
		 * @var object
		...
		const CPT_NAME = 'pstoolkit_email_log';
		...
		public static function register_email_logs_cpt() {
			$labels = array(
				'name'          => _x( 'E-Mail-Protokolle', 'Post Type General Name', 'ub' ),
				'singular_name' => _x( 'E-Mail-Protokoll', 'Post Type Singular Name', 'ub' ),
			);
			$args   = array(
				'label'               => __( 'E-Mail-Protokoll', 'ub' ),
				'description'         => __( 'Beschreibung des Beitragstyps', 'ub' ),
				'labels'              => $labels,
