...
if ( ! class_exists( 'Branda_Maintenance_Maintenance' ) ) {
	...
	class Branda_Maintenance_Maintenance extends PSToolkit_Helper {
		...
		public function __construct() {
			...
			add_filter( 'pstoolkit_settings_maintenance', array( $this, 'admin_options_page' ) );
			add_filter( 'pstoolkit_settings_maintenance_process', array( $this, 'update' ), 10, 1 );
			/**
			 * AJAX
			 */
			add_action( 'wp_ajax_pstoolkit_maintenance_search_sites', array( $this, 'ajax_search_subsite' ) );
			add_action( 'wp_ajax_pstoolkit_maintenance_delete_subsite', array( $this, 'ajax_delete_subsite' ) );
			/**
			 ...
			 * @since 3.0,0
			 */
			add_filter( 'pstoolkit_get_module_content', array( $this, 'add_template' ), 10, 2 );
			/**
			 * Remove some color options
			 *
			 * @since 3.1.0
			 */
			add_filter( 'pstoolkit_get_options_fields_colors_error_messages', array( $this, 'remove_colors' ), 10, 3 );
		}
		...
		protected function set_options() {
			$description = __( 'Wähle den Webseiten-Modus für Deine Webseite.', 'ub' );
			if ( $this->is_network ) {
				$description = __( 'Wähle den Webseiten-Modus gemäß Deinen Anforderungen und anschließend Webseiten in Deinem Netzwerk, um diesen Modus anzuwenden.', 'ub' );
			}
			...
			$options         = array(
					...
					'fields'      => array(
						'mode'     => array(
							...
							'options'     => array(
								...
								'maintenance' => __( 'WARTUNG', 'ub' ),
							),
							'default'     => 'off',
							'description' => __( 'Wähle einen Modus für Deine Webseite.', 'ub' ),
						),
						'search'   => array(
							...
							'placeholder'  => esc_attr__( 'Suche die Unterwebseite', 'ub' ),
							'data'         => array(
								'user-id' => get_current_user_id(),
								'nonce'   => wp_create_nonce( $this->get_nonce_action_name( 'search' ) ),
								'action'  => 'pstoolkit_maintenance_search_sites',
								'extra'   => 'pstoolkit_maintenance_add_already_used_sites',
							),
							'network-only' => true,
						),
						'list'     => array(
						...
						'subsites' => array(
							'type'         => 'sui-tab',
							'label'        => __( 'Anwenden auf', 'ub' ),
							'description'  => __( 'Wähle die Webseiten in Deinem Netzwerk aus, um diesen Modus anzuwenden.', 'ub' ),
							'options'      => array(
								'all'    => __( 'Alle Webseiten', 'ub' ),
								'custom' => __( 'Ausgewählte Webseiten', 'ub' ),
							),
							'default'      => 'all',
				...
				'content' => array(
					'title'       => __( 'Inhalt', 'ub' ),
					'description' => __( 'Passe den Standardinhalt Deiner Infoseite an.', 'ub' ),
					'show-as'     => 'accordion',
					'fields'      => $this->get_options_fields( 'content', array( 'logo', 'error_messages', 'countdown', 'social', 'reset' ) ),
				),
				'design'  => array(
					'title'       => __( 'Design', 'ub' ),
					'description' => __( 'Passe das Standarddesign der Infoseite an.', 'ub' ),
					'show-as'     => 'accordion',
			 ...
			 * @since 3.1.3
			 */
			$options['colors']['fields']['error_messages_background']['accordion']['title'] = __( 'Anzeige Nachricht', 'ub' );
		...
		public function output() {
			...
			if ( empty( $this->data['content']['title'] ) && empty( $this->data['content']['content'] ) ) {
				$this->data['content']['title']   = __( 'Wir werden bald zurück sein!', 'ub' );
				$this->data['content']['content'] = __( 'Entschuldige die Unannehmlichkeiten, aber wir führen derzeit einige Wartungsarbeiten durch. Wir werden in Kürze wieder online sein!', 'ub' );
				if ( 'coming-soon' == $status ) {
					$this->data['content']['title']   = __( 'In Kürze', 'ub' );
					$this->data['content']['content'] = __( 'Bleib dran!', 'ub' );
				}
			 ...
			 * Common: body
			 */
			$css .= $this->common_css_body( 'body.pstoolkit-maintenance' );
			/**
			 * Common: Document
			 */
		...
		public function only_allow_logged_in_rest_access( $access ) {
			...
			if ( version_compare( $current_wp_version, '4.7', '>=' ) ) {
				return new WP_Error( 'rest_cannot_access', __( 'Nur authentifizierte Benutzer können auf die REST-API zugreifen.', 'ub' ), array( 'status' => rest_authorization_required_code() ) );
			}
			return $access;
		}
		...
		public function get_options_fields_content_error_messages( $defaults = array() ) {
			$data = array(
				...
				'content' => array(
					...
					'placeholder' => esc_html__( 'Gib hier Deinen Inhalt ein…', 'ub' ),
				),
			);
			/**
		...
		public function get_options_fields_content_countdown( $defaults = array() ) {
			$data = array(
				'countdown_till_date' => array(
					...
					'label'        => __( 'Bis zum', 'ub' ),
					'description'  => array(
						'content'  => __( 'Wähle das Datum, bis der ausgewählte Webseiten-Modus angewendet wird.', 'ub' ),
						'position' => 'bottom',
					),
					'placeholder'  => esc_attr__( 'Suche einen Termin aus', 'ub' ),
		...
		public function get_options_fields_design_countdown( $defaults = array() ) {
			$data = array(
				'flipclock'          => array(
					...
					'display'      => 'sui-tab-content',
					'content'      => '<div class="pstoolkit-countdown pstoolkit-countdown-flipclock"></div>',
					'accordion'    => array(
						'begin' => true,
						'title' => __( 'Countdown-Timer', 'ub' ),
					),
				...
				'final-countdown'    => array(
					...
					'display'      => 'sui-tab-content',
					'content'      => '<div class="pstoolkit-countdown pstoolkit-countdown-final-countdown"></div>',
				),
				'raw'                => array(
					'type'         => 'raw',
					'master'       => $this->get_name( 'template' ),
					...
					'display'      => 'sui-tab-content',
					'content'      => 'raw',
					'content'      => '<div class="pstoolkit-countdown pstoolkit-countdown-raw"></div>',
				),
		...
		private function get_ace_selectors() {
			$selectors = array(
				'general' => array(
					'selectors' => array(
						'.overall' => __( 'Insgesamt', 'ub' ),
						'.page'    => __( 'Seite', 'ub' ),
						'.clock'   => __( 'Uhr', 'ub' ),
						'.content' => __( 'Inhalt', 'ub' ),
						'#social'  => __( 'Soziale Medien', 'ub' ),
						'#logo'    => __( 'Logo', 'ub' ),
					),
				),
			);
