(function ($) {
	...
	window.PSToolkit.Window_Resize_Manager = {
		on_resize: function (callback) {
			$(window).on('resize.pstoolkit-custom-admin-menu', _.debounce(function () {
				callback();
			}, 200));
		},
		reset: function () {
			$(window).off('resize.pstoolkit-custom-admin-menu');
		}
	};
	...
	var Custom_Admin_Menu_Elements = function (menu_key) {
		...
		this.root = $(markup);
		this.builder = $('> .pstoolkit-custom-admin-menu-builder-fields', this.root);

		this.footer = $('> .sui-box-footer', this.root);
		this.discard_button = $('.pstoolkit-discard-admin-menu-changes', this.footer);
		this.save_button = $('.pstoolkit-apply-admin-menu-changes', this.footer);
	};

	window.PSToolkit.Custom_Admin_Menu = function (menu_key) {
		this.init = init;
		...
		/**
		 * @var {PSToolkit.Menu_Item_Container}
		 */
		var menu_items;
		...
		function clear_window_resize_listeners() {
			PSToolkit.Window_Resize_Manager.reset();
		}
		...
		function handle_discard_button_click() {
			var sure = confirm(pstoolkit_custom_menu.discard_confirm);
			if (!sure) {
				return;
			}
		...
		function scroll_to_first_error() {
			var $dialog = $('#pstoolkit-admin_menu-custom-admin-menu'),
		...
		function display_success_notice() {
			SUI.openFloatNotice( pstoolkit_custom_menu.setting_updated, 'success' );
		}
		...
		function set_loading_icon_visibility(visible) {
			var loading_class = 'pstoolkit-admin-menu-loading';
		...
		function fetch_menu_data() {
			return post('pstoolkit_admin_bar_load_menu', {});
		}
		...
		function post(action, extra_data) {
			var deferred = new $.Deferred(),
				data = $.extend({
					action: action,
					key: menu_key,
					_wpnonce: pstoolkit_custom_menu.nonce
				}, extra_data);
		...
		function initialize_items(data) {
			...
			menu_items = new PSToolkit.Menu_Item_Container();
			...
			for (var key in menu_data) {
				...
				menu_items.add(
					new PSToolkit.Menu_Item(key, menu_data[key], menu_items)
				);
			}
		}
		...
		function save_menu_and_reload() {
			...
			return do_menu_action_and_reload('pstoolkit_admin_bar_save_menu', {
				menu: menu_data
			});
		}

		function delete_menu_and_reload() {
			return do_menu_action_and_reload('pstoolkit_admin_bar_remove_menu');
		}
		 ...
		 */
		function remove() {
			return post('pstoolkit_admin_bar_remove_menu', {});
		}
