 ...
 * @subpackage Settings
 */
if ( ! class_exists( 'PSToolkit_Import' ) ) {

	/**
	 * Class PSToolkit_Import.
	 */
	class PSToolkit_Import extends PSToolkit_Helper {
		...
		/**
		 * PSToolkit_Import constructor.
		 */
		public function __construct() {
			parent::__construct();
			$this->module = 'import';
			// Register all hooks for the module.
			add_filter( 'pstoolkit_settings_import', array( $this, 'admin_options_page' ) );
			add_filter( 'pstoolkit_settings_import_process', array( $this, 'update' ) );
			add_filter( 'pstoolkit_handle_group_page', array( $this, 'steps' ), 10, 2 );
			add_filter( 'pstoolkit_sui_wrap_class', array( $this, 'container_classes' ), 10, 2 );
			/**
			 *Hooks on error page
			 */
			add_filter( 'pstoolkit_change_footer', array( $this, 'hide_footer_links' ), 10, 2 );
			add_filter( 'pstoolkit_footer_text', array( $this, 'change_footer_text' ), 10, 2 );
			add_filter( 'pstoolkit_sui_wrap_class', array( $this, 'add_error_class' ), 10, 2 );
		}
		...
		public function steps( $content, $module ) {
			...
			if ( 'error' === $key ) {
				...
				$args     = array(
					'filename'   => isset( $data['filename'] ) && ! empty( $data['filename'] ) ? $data['filename'] : _x( 'Unbekannt', 'Dateiname auf dem Importfehlerbildschirm.', 'ub' ),
		...
		private function import_step_2( $content, $key, $data ) {
			...
			foreach ( $configuration as $id => $module ) {
				if ( $pstoolkit_network && ! is_network_admin() ) {
		...
		private function import_step_3( $content, $key, $data ) {
			...
			if (
				 ...
				 */
				$file = pstoolkit_files_dir( 'class-download-remote-image.php' );
				include_once $file;
				foreach ( $configuration as $id => $module ) {
			...
			if ( 0 < $number ) {
				$message = array(
					...
					'message' => sprintf(
						_n(
							'%s Modul erfolgreich importiert und konfiguriert.',
							'%s Module erfolgreich importiert und konfiguriert.',
							$number,
							'ub'
						),
						$this->bold( $number )
			 ...
			 * update import stats
			 */
			do_action( 'pstoolkit_admin_stats_write', 'import' );
		}
				...
				'import' => array(
					...
					'fields'      => array(
						'file'     => array(
							...
							'name'        => 'import',
							'description' => array(
								'content'  => __( 'Wähle eine JSON-Datei (.json), um die Konfigurationen zu importieren.', 'ub' ),
								'position' => 'bottom',
							),
						),
						'button'   => array(
							'type'     => 'submit',
							'value'    => __( 'Importieren', 'ub' ),
							'disabled' => true,
							'sui'      => array(
								'blue',
							),
		...
		private function image( $url ) {
			...
			return new WP_Error( 'broke', __( 'PSToolkit konnte kein Bild importieren.', 'ub' ) );
		}
	}

}
new PSToolkit_Import();
