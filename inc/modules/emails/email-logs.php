...
if ( ! class_exists( 'PSToolkit_Email_Logs' ) ) {

	class PSToolkit_Email_Logs extends PSToolkit_Helper {
		...
		public function __construct() {
			...
			$this->module_name = PSToolkit_Helper::hyphen_to_underscore( $this->module );

			add_filter( 'pstoolkit_settings_' . $this->module_name, array( $this, 'admin_options_page' ) );
			add_filter( 'pstoolkit_settings_' . $this->module_name . '_process', array( $this, 'update' ) );
			...
			add_filter( 'pstoolkit_settings_panel_show_submit', array( $this, 'disable_save_changes_button' ), 10, 2 );

			// Change module content.
			add_filter( 'pstoolkit_get_module_content', array( $this, 'change_main_content' ), 10, 2 );

			add_action( 'init', array( 'PSToolkit_Email_Logs_CPT', 'register_email_logs_cpt' ), 0 );

			// `Delete` dialog attrs.
			add_filter( 'pstoolkit_dialog_delete_attr', array( $this, 'dialog_delete_attr_filter' ), 10, 3 );

			// Handle Delete log(s).
			add_action( 'wp_ajax_pstoolkit_email_logs_delete', array( $this, 'ajax_delete' ) );
			add_action( 'wp_ajax_pstoolkit_email_logs_delete_bulk', array( $this, 'ajax_delete_bulk' ) );
			...
			// Handle crone action.
			add_action( 'pstoolkit_email_logs_cleaning', array( $this, 'email_logs_cleaning' ) );
		...
		public static function add_schedule() {
			if ( ! wp_next_scheduled( 'pstoolkit_email_logs_cleaning' ) ) {
				wp_schedule_event( time(), 'hourly', 'pstoolkit_email_logs_cleaning' );
			}
		}
		...
		public function email_logs_cleaning() {
			...
			$items          = PSToolkit_Email_Logs_CPT::get_items( $args );
			...
			foreach ( $ids_for_remove as $id ) {
				PSToolkit_Email_Logs_CPT::delete_post( $id );
			}
		}
		...
		public function enqueue_scripts( $module ) {
			...
			wp_enqueue_script(
				'pstoolkit-moment-js',
				pstoolkit_url( 'assets/js/vendor/moment.min.js' ),
				array( 'jquery' ),
				$this->build,
				true
			);
			...
			wp_enqueue_script(
				'pstoolkit-datepicker-range',
				pstoolkit_url( 'assets/js/vendor/daterangepicker.min.js' ),
				array( 'jquery', 'pstoolkit-moment-js' ),
				$this->build,
				true
			);
			...
			$daterangepicker_ranges = sprintf(
				"
				var pstoolkit_datepicker_ranges = {
					'%s': [moment(), moment()],
				...
				__( 'Heute', 'ub' ),
				__( 'Gestern', 'ub' ),
				__( 'Letzten 7 Tage', 'ub' ),
				__( 'Letzten 30 Tage', 'ub' ),
				__( 'Dieser Monat', 'ub' ),
				__( 'Letzter Monat', 'ub' )
			);
			 ...
			 */
			$daterangepicker_ranges = apply_filters( 'pstoolkit_datepicker_ranges', $daterangepicker_ranges );

			wp_add_inline_script( 'pstoolkit-datepicker-range', $daterangepicker_ranges );

			$locale = self::get_locale();
			wp_localize_script( 'pstoolkit-datepicker-range', 'pstoolkit_datepicker_locale', $locale );
		}
		...
		public static function make_email_clickable( $email ) {
			if ( is_email( $email ) ) {
				$email = '<a href="mailto:' . $email . '" target="_blank" title="' . __( 'E-Mail senden', 'ub' ) . '">' . $email . '</a>';
			}
		...
		protected function set_options() {
			$options = array(
				...
				'export'         => array(
					'fields' => array(
						'items' => array(
							'label'       => __( 'Protokolle exportieren', 'ub' ),
							'description' => __( 'Exportiere alle E-Mail-Protokolldaten in eine CSV-Datei.', 'ub' ),
							'type'        => 'submit',
							'icon'        => 'download',
							'name'        => 'pstoolkit_email_log_export',
							'classes'     => array(
								'pstoolkit-export-email-logs',
							),
							'value'       => __( 'Exportieren', 'ub' ),
						),
						'nonce' => array(
							'type'  => ' hidden',
				...
				'clear'          => array(
					'fields' => array(
						'items'  => array(
							...
							'label'        => __( 'Protokolllimit', 'ub' ),
							'description'  => array(
								'content'  => __( 'Setze das Limit auf 0, wenn Du Deinen gesamten Protokollverlauf behalten möchtest.', 'ub' ),
								'position' => 'bottom',
							),
						...
						'border' => array(
							'type'        => 'sui-tab',
							'label'       => __( 'Protokoll löschen', 'ub' ),
							'description' => __( 'Lege das Limit fest, nach dem der Verlauf der Protokolle gelöscht wird.', 'ub' ),
							'options'     => array(),
							'default'     => 'clear_log',
		...
		public function change_main_content( $content, $module ) {
			...
			/*if ( ! PSToolkit_Helper::is_full_pro() ) {
				...
				$total = PSToolkit_Email_Logs_CPT::get_total_cpt();
		...
		public function update( $status ) {
			// Email Logs export.
			$pstoolkit_email_log_export = filter_input( INPUT_POST, 'pstoolkit_email_log_export' );
			if ( ! is_null( $pstoolkit_email_log_export ) ) {
		...
		public function singular_email_log_export() {
			// Email Logs export.
			$pstoolkit_email_log_export = filter_input( INPUT_GET, 'pstoolkit_email_log_export' );
			if ( ! is_null( $pstoolkit_email_log_export ) ) {
				$uba = pstoolkit_get_uba_object();

				$id    = filter_input( INPUT_POST, 'email_log_id', FILTER_VALIDATE_INT );
				$nonce = filter_input( INPUT_POST, 'pstoolkit_nonce' );
		...
		public function dialog_delete_attr_filter( $args, $module, $id ) {
			if ( $this->module === $module ) {
				$args['title']       = __( 'Protokoll löschen', 'ub' );
				$args['description'] = __( 'Möchtest Du dieses Protokoll wirklich dauerhaft löschen?', 'ub' );
				if ( 'bulk' === $id ) {
					$args['title']       = __( 'Protokolle löschen', 'ub' );
					$args['description'] = __( 'Möchtest Du diese Protokolle wirklich dauerhaft löschen?', 'ub' );
				}
			}
			return $args;
		}
		...
		public function ajax_delete() {
			...
			$message = array(
				'type'    => 'success',
				'message' => esc_html__( 'E-Mail-Protokoll wurde gelöscht.', 'ub' ),
			);
			$uba     = pstoolkit_get_uba_object();
		...
		public function ajax_delete_bulk() {
			...
			if ( ! is_array( $ids ) ) {
				...
				$message = array(
					'type'    => 'success',
					'message' => esc_html__( 'Ausgewählte E-Mail-Protokolle wurden gelöscht!', 'ub' ),
				);
				$uba->add_message( $message );
				wp_send_json_success();
			}
		...
		private function email_logs_table() {
			...
			$all_filtered_items = PSToolkit_Email_Logs_CPT::get_items( $main_args );
			$filter_top_args    = array_merge(
				...
				array(
					...
					'order_array'    => array(
						'desc' => esc_html__( 'Absteigend', 'ub' ),
						'asc'  => esc_html__( 'Aufsteigend', 'ub' ),
					),
				)
			);

			$filter_bottom_args = array_merge( $filter_top_args, array( 'is_bottom' => true ) );
			$items              = PSToolkit_Email_Logs_CPT::items_slice( $all_filtered_items, $main_args );
			...
			$args               = array(
				...
				'export_form_action' => add_query_arg( array( 'pstoolkit_email_log_export' => true ) ),
				'module_object'      => $this,
			);
		...
		private static function get_default_cpt_args() {
			$limit = apply_filters( 'pstoolkit_email_logs_pagination_limit', 10 );
			$args  = array(
				'order_by'    => 'date',
				'order'       => 'desc',
				'limit'       => $limit,
		...
		private static function get_default_columns() {
			$defalt_columns = array(
				...
				'recipient'  => __( 'Empfänger', 'ub' ),
				'date'       => __( 'Gesendet am', 'ub' ),
				'post_title' => __( 'Betreff', 'ub' ),
			);
		...
		protected static function maybe_add_smtp_notice() {
			$is_smtp_activated = pstoolkit_is_active_module( 'emails/smtp.php' );
			if ( ! $is_smtp_activated ) {
				...
				$notice  = sprintf( esc_html__( 'Aktiviere das %1$sSMTP-Moduls%2$s, um den E-Mail-Protokollverlauf zu speichern.', 'ub' ), '<a href="' . esc_url( $url ) . '">', '</a>' );
				$content = PSToolkit_Helper::sui_notice( $notice, 'default' );
			} else {
				$content = '';
			}
