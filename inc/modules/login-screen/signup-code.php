<?php
/**
 * PSToolkit Signup Codes class.
 *
 * @package PSToolkit
 * @subpackage Front-end
 */
if ( ! class_exists( 'PSToolkit_Signup_Codes' ) ) {

	class PSToolkit_Signup_Codes extends PSToolkit_Helper {
		...
		public function __construct() {
			...
			add_filter( 'pstoolkit_settings_signup_code', array( $this, 'admin_options_page' ) );
			add_filter( 'pstoolkit_settings_signup_code_process', array( $this, 'update' ) );
			add_filter( 'pstoolkit_settings_signup_code_process', array( $this, 'update_items' ), 1 );
			add_filter( 'pstoolkit_settings_signup_code_preserve', array( $this, 'add_preserve_fields' ) );
			/**
			 * User Create Code
			 */
		...
		protected function set_options() {
			$options = array(
				'user' => array(
					'title'       => __( 'Benutzer Registration', 'ub' ),
					'description' => __( 'Wähle aus ob sich jemand auf Deiner Webseite registrieren kann oder ob Du die Registrierung nur mit einem Anmeldecode einschränken möchtest.', 'ub' ),
					'fields'      => array(
						'items'    => array(
							'type'         => 'callback',
						...
						'help'     => array(
							'type'         => 'text',
							'label'        => __( 'Feld Beschreibung', 'ub' ),
							'description'  => array(
								'content'  => __( 'Dies wird unter dem Eingabefeld im Anmeldeformular angezeigt.', 'ub' ),
								'position' => 'bottom',
							),
							'default'      => __( 'Du musst den Code eingeben, um einen Benutzer zu erstellen.', 'ub' ),
							'master'       => $this->get_name( 'user' ),
							'master-value' => 'on',
							'display'      => 'sui-tab-content',
						),
						'error'    => array(
							'type'         => 'text',
							'label'        => __( 'Fehlermeldung', 'ub' ),
							'description'  => array(
								'content'  => __( 'Dies wird unter dem Eingabefeld im Anmeldeformular angezeigt.', 'ub' ),
								'position' => 'bottom',
							),
							'default'      => __( '<strong>FEHLER</strong>: Der von Dir eingegebene Code ist ungültig.', 'ub' ),
							'master'       => $this->get_name( 'user' ),
							'master-value' => 'on',
							'display'      => 'sui-tab-content',
						),
				...
				'blog' => array(
					'title'        => __( 'Blog-Registrierung', 'ub' ),
					'description'  => __( 'Wähle ob jemand ein Blog auf Deiner Webseite registrieren kann oder ob Du die Registrierung nur mit einem Anmeldecode einschränken möchtest.', 'ub' ),
					'network-only' => true,
					'fields'       => array(
						'items'    => array(
							'type'         => 'callback',
						...
						'branding' => array(
							'type'         => 'text',
							'label'        => __( 'Feldbezeichnung', 'ub' ),
							'description'  => array(
								'content'  => __( 'Diese Beschriftung wird im Anmeldeformular mit dem Anmeldecodefeld angezeigt.', 'ub' ),
								'position' => 'bottom',
							),
							'default'      => __( 'Blog erstellen-Code', 'ub' ),
							'master'       => $this->get_name( 'blog' ),
							'master-value' => 'on',
							'display'      => 'sui-tab-content',
						),
						'help'     => array(
							...
							'default'      => __( 'Du musst den Code eingeben, um einen Blog zu erstellen.', 'ub' ),
							'master'       => $this->get_name( 'blog' ),
							'master-value' => 'on',
							'display'      => 'sui-tab-content',
						),
						'error'    => array(
							...
							'default'      => __( 'Blog-Erstellungscode ist ungültig.', 'ub' ),
							'master'       => $this->get_name( 'blog' ),
							'master-value' => 'on',
							'display'      => 'sui-tab-content',
						),
		...
		private function print_field( $id, $value, $errors ) {
			echo '<p class="pstoolkit-password">';
			$html_id = 'pstoolkit_' . $id;
		...
		public function add_user_code( $errors ) {
			...
			global $pstoolkit_bp_errors;
			if ( ! empty( $pstoolkit_bp_errors['pstoolkit-signup-code-user_code'] ) && function_exists( 'nouveau_error_template' ) ) {
				$errors           = array( 'buddypress' => nouveau_error_template( wp_strip_all_tags( $pstoolkit_bp_errors['pstoolkit-signup-code-user_code'] ), 'bp-feedback error' ) );
				$pstoolkit_bp_errors = null;
			}
			...
			if ( ! isset( $value['branding'] ) || empty( $value['branding'] ) ) {
				$value['branding'] = __( 'Benutzer erstellen-Code', 'ub' );
			}
			/**
			 * print
			 */
		...
		public function add_wrong_code_error() {
			global $pstoolkit_bp_errors;
			if ( ! empty( $pstoolkit_bp_errors['pstoolkit-signup-code-user_code'] ) && function_exists( 'buddypress' ) ) {
				$bp = buddypress();
				$bp->signup->errors['pstoolkit-signup-code-user_code'] = $pstoolkit_bp_errors['pstoolkit-signup-code-user_code'];
			}
		}
			...
			if ( $show ) {
				...
				if ( ! $match ) {
					if ( function_exists( 'buddypress' ) ) {
						global $pstoolkit_bp_errors;
						$pstoolkit_bp_errors[ $name ] = $this->get_value( 'user', 'error' );
					}
		...
		public function add_blog_code( $errors ) {
			...
			if ( ! isset( $value['branding'] ) || empty( $value['branding'] ) ) {
				$value['branding'] = __( 'Blog erstellen-Code', 'ub' );
			}
			/**
			 * print
			 */
		...
		public function button_add() {
			$args = array(
				...
				'icon' => 'plus',
				'text' => _x( 'Anmeldecode hinzufügen', 'button', 'ub' ),
				'sui'  => 'blue',
			);
		...
		public function add_dialog( $content, $module ) {
			 ...
			 */
			$roles    = array(
				'-' => __( 'Wähle eine Benutzerrolle', 'ub' ),
			);
			$roles   += $this->roles;
			...
			foreach ( $types as $type ) {
				...
				if (
					...
					foreach ( $_POST['simple_options'][ $type ] as $key => $data ) {
						if ( preg_match( '/^(new|pstoolkit)/', $key ) ) {
