...
class PSToolkit_Admin_Menu_Builder {
	private $menu;
	private $markup = '';
	...
	private function print_collapse_button() {
		...
		<li id="collapse-menu" class="hide-if-no-js">
			<button type="button" id="collapse-button" aria-label="<?php echo esc_attr__( 'Hauptmenü reduzieren', 'ub' ); ?>"
					aria-expanded="true">
				<span class="collapse-button-icon" aria-hidden="true"></span>
				<span class="collapse-button-label"><?php echo __( 'Menü reduzieren', 'ub' ); ?></span>
			</button>
		</li>
		<?php
	}
	...
	private function get_menu_item_class_attribute( $menu_item_id, $menu_item_settings, $is_first ) {
		...
		$custom_classes = pstoolkit_get_array_value( $menu_item_settings, 'css_classes' );
	...
	private function get_submenu_class_attribute( $submenu_item_id, $submenu_item_settings, $main_item_slug, $is_first, $has_icon ) {
		...
		if ( $has_icon ) {
			$class[] = 'pstoolkit-sub-menu-with-icon';
		}
		...
		$custom_classes = pstoolkit_get_array_value( $submenu_item_settings, 'css_classes' );
		if ( ! empty( $custom_classes ) ) {
			$class[] = esc_attr( $custom_classes );
		}
	...
	private function get_image( $menu_item_settings, $container_class, $svg_style, $default_value_empty ) {
		...
		if ( $icon_type === 'none' ) {
			...
			if ( $icon_image_id ) {
				...
				if ( ! image_downsize( $icon_image_id ) && $pstoolkit_network && ! is_main_site() ) {
	...
	private function print_menu_start() {
		?>
	<div id="adminmenumain" role="navigation" aria-label="<?php esc_attr_e( 'Hauptmenü', 'ub' ); ?>">
		<a href="#wpbody-content" class="screen-reader-shortcut"><?php _e( 'Zum Hauptinhalt springen', 'ub' ); ?></a>
		<a href="#wp-toolbar" class="screen-reader-shortcut"><?php _e( 'Zur Symbolleiste springen', 'ub' ); ?></a>
