<?php
/**
 * PSToolkit Images class.
 *
 * @package PSToolkit
 * @subpackage Utilites
 */
if ( ! class_exists( 'PSToolkit_Images' ) ) {

	class PSToolkit_Images extends PSToolkit_Helper {
		...
		public function __construct() {
			 ...
			 * hooks
			 */
			add_filter( 'pstoolkit_settings_images', array( $this, 'admin_options_page' ) );
			add_filter( 'pstoolkit_settings_images_process', array( $this, 'update' ) );
			add_filter( 'pstoolkit_settings_images_process', array( $this, 'update_blogs_icons' ) );
			add_filter( 'pstoolkit_settings_images_process', array( $this, 'update_upload_limits' ), 11 );
			/**
			 * upgrade options
			 *
			 * @since 1.0.0
			 ...
			 * AJAX
			 */
			add_action( 'wp_ajax_pstoolkit_images_search_sites', array( $this, 'ajax_search_subsite' ) );
			add_action( 'wp_ajax_pstoolkit_images_delete_subsite', array( $this, 'ajax_delete_subsite' ) );
			/******************************************************************
			 ...
			 * @since x.x.x
			 */
			add_filter( 'pstoolkit_localize_script', array( $this, 'localize_script' ), 10, 2 );
		}
		...
		public function upgrade_options() {
			...
			$data   = pstoolkit_get_option( 'ub_favicons' );
			if ( ! empty( $data ) ) {
				...
				pstoolkit_delete_option( 'ub_favicons' );
			}
			if ( $update ) {
		...
		protected function set_options() {
			 ...
			 */
			$override_description = __( 'Wähle aus ob das hier definierte Favicon das in <b>Darstellung&gt; Anpassen</b> überschreibt.', 'ub' );
			if ( ! $this->is_network ) {
			...
			$options = array(
				'favicon' => array(
					'title'       => __( 'Favicon', 'ub' ),
					'description' => $this->is_network ? __( 'Du kannst die Favicons aller Webseiten in Deinem Netzwerk überschreiben.', 'ub' ) : __( 'Du kannst das in <strong>Darstellung &gt; Anpassen</strong> definierte Favicon Deiner Webseite überschreiben.', 'ub' ),
					'fields'      => array(
						'favicon'  => array(
							'label'       => $this->is_network ? __( 'Hauptseite', 'ub' ) : __( 'Favicon', 'ub' ),
							'type'        => 'media',
							'master'      => $this->get_name( 'favicons-override' ),
							'description' => array(
								'content'  => $this->is_network ? __( 'Überschreibe hier das Favicon Deiner Hauptseite. Die bevorzugte Größe des Favicons ist 32x32px.', 'ub' ) : __( 'Die bevorzugte Größe des Favicons ist 32x32px.', 'ub' ),
						...
						'search'   => array(
							...
							'placeholder'  => esc_attr__( 'Suche Unterwebseite', 'ub' ),
							'data'         => array(
								'user-id' => get_current_user_id(),
								'nonce'   => wp_create_nonce( $this->get_nonce_action_name( 'search' ) ),
								'action'  => 'pstoolkit_images_search_sites',
								'extra'   => 'pstoolkit_images_add_already_used_sites',
							),
							'network-only' => true,
						),
						'list'     => array(
							...
							'display'      => 'sui-tab-content',
							'network-only' => true,
							'classes'      => array( 'pstoolkit-images-subsite-add' ),
						),
						'subsites' => array(
							'type'         => 'sui-tab',
							'description'  => array(
								'content' => __( 'Wähle aus, ob das Favicon der Hauptwebseite als Standardfavicon für alle Unterwebseiten verwendet werden soll, oder füge für jede Unterwebseite ein benutzerdefiniertes Favicon hinzu.', 'ub' ),
							),
							'label'        => __( 'Unterwebseiten', 'ub' ),
							'options'      => array(
								'force'  => __( 'Favicon der Hauptwebseite', 'ub' ),
				...
				'images'  => array(
					...
					'fields'      => array(
						...
						'limits'   => array(
							...
							'description'       => array(
								'position' => 'top',
								'content'  => __( 'Lege Dein eigenes Limit für die Upload-Größe von Bildern für verschiedene Benutzerrollen fest.', 'ub' ),
							),
						),
					),
				),
			...
			if ( $this->is_network ) {
				...
				if ( ! is_network_admin() ) {
					$subsite = apply_filters( 'pstoolkit_module_check_for_subsite', false, $this->module, null );
					if ( $subsite ) {
		...
		public function manage_sites_custom_column( $column, $site_id ) {
			...
			$text    = esc_html__( 'Ändern', 'ub' );
			if ( empty( $favicon ) ) {
				$text = esc_html__( 'Setzen', 'ub' );
			} else {
		...
		public function get_list() {
			...
			$content .= PSToolkit_Helper::sui_notice(
				__( 'Du hast keine Unterwebseite hinzugefügt, um das Favicon zu überschreiben. Durchsuche die Unterwebseiten und klicke auf Hinzufügen.', 'ub' ),
				$notice_class
			);
			return $content;
		}
		...
		public function get_fs_limit( $current_limit ) {
			...
			if ( ! isset( $limits['super'] ) && is_super_admin() && ! is_network_admin() ) {
				...
				$network_settings = pstoolkit_get_option( 'ub_images', false, 'normal', true );
