<?php
/**
 * PSToolkit Login Screen class.
 *
 * @package PSToolkit
 * @subpackage Front-end
 */
if ( ! class_exists( 'PSToolkit_Login_Screen' ) ) {

	/**
	 * Class PSToolkit_Login_Screen.
	 */
	class PSToolkit_Login_Screen extends PSToolkit_Helper {
		...
		/**
		 * PSToolkit_Login_Screen constructor.
		 */
		public function __construct() {
			parent::__construct();
			$this->module = 'login-screen';
			add_filter( 'pstoolkit_settings_login_screen', array( $this, 'admin_options_page' ) );
			add_filter( 'pstoolkit_settings_login_screen_process', array( $this, 'update' ), 10, 1 );
			add_filter( 'pstoolkit_settings_login_screen_preserve', array( $this, 'add_preserve_fields' ) );
			...
			add_filter( 'pstoolkit_reset_section_login-screen', array( $this, 'reset_section' ), 10, 3 );
			add_action( 'pstoolkit_helper_admin_options_page_before_options', array( $this, 'before_admin_options_page' ) );
			/**
			 * Add login message
			 *
			 * @since 3.2.0
			 ...
			 * Set template
			 */
			add_action( 'wp_ajax_pstoolkit_login_screen_set_template', array( $this, 'ajax_set_template' ) );
			/**
			 * Validate password
			 *
			 * @since x.x.x
		...
		public function upgrade_options_module_custom_login_css() {
			global $ub_version;
			$compare = version_compare( $ub_version, '2.3.0' ); 
			if ( 0 < $compare ) {
				// Move settings from "Login CSS" module.
				$module_name = 'custom-login-css.php';
				$is_active   = pstoolkit_is_active_module( $module_name );
				if ( $is_active ) {
					$value = pstoolkit_get_option( 'global_login_css' );
					if ( ! empty( $value ) ) {
						if (
							is_array( $value )
							&& isset( $value['login'] )
						...
						pstoolkit_delete_option( 'global_login_css' );
		...
		protected function set_options() {
			...
			$options = array(
				'theme'    => array(
					'title'       => __( 'Vorlage', 'ub' ),
					'description' => __( 'Passe eine unserer vorgefertigten Vorlagen an oder gestalte die Anmeldeseite von Grund auf neu.', 'ub' ),
					'fields'      => array(
						'id' => array(
							'type'     => 'callback',
				...
				'content'  => array(
					'title'       => __( 'Inhalt', 'ub' ),
					'description' => __( 'Passe den Standardinhalt des Anmeldebildschirms an.', 'ub' ),
					'show-as'     => 'accordion',
				...
				'design'   => array(
					'title'       => __( 'Design', 'ub' ),
					'description' => __( 'Passe das Standarddesign des Anmeldebildschirms an.', 'ub' ),
					'show-as'     => 'accordion',
					'fields'      => $this->get_options_fields(
						'design',
						array(
				...
				// Redirects.
				'redirect' => array(
					'title'       => __( 'Umleitung', 'ub' ),
					'description' => __( 'Wähle aus, wohin Du Benutzer nach erfolgreicher An- oder Abmeldung umleiten möchtest.', 'ub' ),
					'fields'      => array(
						'login_url'  => array(
							'type'         => 'url',
							'label'        => __( 'Nach Anmeldung URL', 'ub' ),
							'master'       => $this->get_name( 'login-related' ),
							'master-value' => 'on',
							'display'      => 'sui-tab-content',
							'description'  => array(
								'content'  => __( 'Du kannst relative oder absolute interne URLs verwenden.', 'ub' ),
								'position' => 'bottom',
							),
						),
						'login'      => array(
							'type'        => 'sui-tab',
							'label'       => __( 'Anmeldung', 'ub' ),
							'description' => __( 'Wähle die URL aus, auf die die Benutzer nach erfolgreicher Anmeldung umgeleitet werden.', 'ub' ),
							'options'     => array(
								'off' => __( 'Standard', 'ub' ),
								'on'  => __( 'Benutzerdefiniert', 'ub' ),
							),
							'default'     => 'off',
						...
						'logout_url' => array(
							'type'         => 'url',
							'label'        => __( 'Abmelden URL', 'ub' ),
							'master'       => $this->get_name( 'logout-related' ),
							'master-value' => 'on',
							'display'      => 'sui-tab-content',
							'description'  => array(
								'content'  => __( 'Du kannst relative oder absolute URLs verwenden, und die Weiterleitung kann sowohl auf eine interne als auch auf eine externe Seite erfolgen.', 'ub' ),
								'position' => 'bottom',
							),
						),
						'logout'     => array(
							'type'        => 'sui-tab',
							'label'       => __( 'Abmeldung', 'ub' ),
							'description' => __( 'Wähle die URL aus, auf die die Benutzer nach erfolgreicher Abmeldung umgeleitet werden.', 'ub' ),
							'options'     => array(
								'off' => __( 'Standard', 'ub' ),
								'on'  => __( 'Benutzerdefiniert', 'ub' ),
							),
				...
				'css'      => $this->get_custom_css_array(
					array(
						'extra_description' => __( 'Dies wird dem Header jeder Anmeldeseite hinzugefügt.', 'ub' ),
		...
		private function get_themes() {
			...
			foreach ( $themes as $k => $value ) {
				$themes[ $k ]['pstoolkit_id'] = $this->get_name( $k );
			}
			return $themes;
		}
		...
		private function get_theme_data( $id ) {
			...
			if ( empty( $data ) ) {
				$message = sprintf(
					__( 'Fehler beim Laden der Vorlagenkonfiguration "%s"!', 'ub' ),
					$theme['Name']
				);
		...
		public function get_options_fields_content_language( $defaults = array() ) {
			 ...
			 */
			$languages = array(
				'default' => __( 'Webseite Standard', 'ub' ),
			);
		...
		public function get_options_fields_content_form( $defaults = array() ) {
			$data = array(
				'form_label_username'    => array(
					'type'      => 'text',
					'label'     => __( 'Eingabebezeichnung für den Benutzernamen', 'ub' ),
					'default'   => __( 'Benutzername oder E-mail Adresse', 'ub' ),
					'accordion' => array(
						'begin' => true,
				...
				'form_label_password'    => array(
					'type'    => 'text',
					'label'   => __( 'Passworteingabeetikett', 'ub' ),
					'default' => __( 'Passwort', 'ub' ),
				),
				'form_label_log_in'      => array(
					'type'    => 'text',
					'label'   => __( 'Text Anmeldeschaltfläche', 'ub' ),
					'default' => __( 'Einloggen', 'ub' ),
				),
				'form_check_remember_me' => array(
					'label'        => __( 'Standardwert', 'ub' ),
					'type'         => 'sui-tab',
					'options'      => array(
						'off' => __( 'Nicht erinnern', 'ub' ),
						'on'  => __( 'Erinnern', 'ub' ),
					),
					'default'      => 'off',
				...
				'form_show_remember_me'  => array(
					'type'        => 'sui-tab',
					'label'       => __( 'Kontrollkästchen "Erinnere dich an mich"', 'ub' ),
					'options'     => array(
				...
				'password_notice'        => array(
					...
					'value'        => PSToolkit_Helper::sui_notice(
						__( 'Hinweis: Diese Einstellung fügt das Kennwortfeld zum Standard-WP-Anmeldeformular hinzu. Wenn Du ein benutzerdefiniertes Anmeldeformular verwendest, füge das Kennwortfeld im Formulareditor hinzu.', 'ub' ),
						'default'
					),
				...
				'form_signup_password'   => array(
					'label'       => __( 'Passwortfeld auf dem Registerbildschirm', 'ub' ),
					'type'        => 'sui-tab',
					'options'     => array(
						'off' => __( 'Hide', 'ub' ),
						'on'  => __( 'Show', 'ub' ),
		...
		public function get_options_fields_content_error_messages( $defaults = array() ) {
			// Invalid username.
			$invalid_username  = __( '<strong>FEHLER</strong>: Ungültiger Benutzername.', 'ub' );
			$invalid_username .= ' <a href="WP_LOSTPASSWORD_URL">';
			$invalid_username .= __( 'Passwort vergessen?', 'ub' );
			$invalid_username .= '</a>';
			// Invalid password.
			$invalid_password  = __( '<strong>FEHLER</strong>: Das Passwort, das Du für den Benutzernamen %s eingegeben hast, ist falsch.', 'ub' );
			$invalid_password .= ' <a href="WP_LOSTPASSWORD_URL">';
			$invalid_password .= __( 'Passwort vergessen?', 'ub' );
			$invalid_password .= '</a>';
			$data              = array(
				'content_empty_username'     => array(
					'type'      => 'textarea',
					'label'     => __( 'Leerer Benutzername', 'ub' ),
					'default'   => __( '<strong>FEHLER</strong>: Das Feld Benutzername ist leer.', 'ub' ),
					'accordion' => array(
						'begin'   => true,
				...
				'content_empty_password'     => array(
					'type'    => 'textarea',
					'label'   => __( 'Passwort leeren', 'ub' ),
					'default' => __( '<strong>FEHLER</strong>: Das Passwortfeld ist leer.', 'ub' ),
				),
		...
		public function get_options_fields_content_links_below_form( $defaults = array() ) {
			$data = array(
				'content_show_register' => array(
					'type'      => 'sui-tab',
					'label'     => __( '"Registrieren | Passwort verloren?" Links', 'ub' ),
					'options'   => array(
						'off' => __( 'Ausblenden', 'ub' ),
						'on'  => __( 'Anzeigen', 'ub' ),
					),
					...
					'accordion' => array(
						'begin'   => true,
						'title'   => __( 'Links unter dem Formular', 'ub' ),
						'classes' => array(
							'body' => array( $this->get_name( 'error-messages' ) ),
						),
					),
				...
				'content_show_policy'   => array(
					'type'      => 'sui-tab',
					'label'     => __( 'Link "Datenschutzerklärung"', 'ub' ),
					'options'   => array(
						'off' => __( 'Ausblenden', 'ub' ),
						'on'  => __( 'Anzeigen', 'ub' ),
					),
		...
		public function get_options_fields_design_form( $defaults = array() ) {
			...
			$data   = array(
				...
				'form_button_rounded'      => array(
					'type'         => 'number',
					'label'        => __( 'Ecken mit Kantenradius', 'ub' ),
					'min'          => 0,
					'default'      => 3,
					'after_label'  => __( 'px', 'ub' ),
					'before_field' => '<div class="sui-col">',
				...
				'form_button_text_shadow'  => array(
					'type'         => 'sui-tab',
					'label'        => __( 'Textschatten', 'ub' ),
					'options'      => array(
						'flat'   => __( 'Flach', 'ub' ),
						'shadow' => __( 'Schattenhafter Text', 'ub' ),
					),
					'default'      => 'shadow',
					'before_field' => '<div class="sui-col">',
					'after_field'  => '</div></div>',
				...
				'form_background_size'     => array(
					'type'      => 'sui-tab',
					'label'     => __( 'Bildanpassung', 'ub' ),
					'options'   => array(
						'cover'   => __( 'Cover', 'ub' ),
						'fill'    => __( 'Füllen', 'ub' ),
						'contain' => __( 'Enthalten', 'ub' ),
						'none'    => __( 'Keine', 'ub' ),
					),
					'default'   => 'cover',
					'group'     => array(
		...
		protected function get_options_fields_colors_form( $defaults = array() ) {
			$data = array(
				...
				'form_input_color'              => array(
					'type'    => 'color',
					'label'   => __( 'Eingabefarbe', 'ub' ),
					'default' => '#32373c',
				),
				'form_input_border'             => array(
					'type'    => 'color',
					'label'   => __( 'Eingaberahmen', 'ub' ),
					'default' => '#ddd',
				),
				'form_input_background'         => array(
					'type'    => 'color',
					'label'   => __( 'Eingabe Hintergrund', 'ub' ),
					'default' => '#fbfbfb',
				),
				'form_button_label'             => array(
					'type'    => 'color',
					'label'   => __( 'Schaltflächenbeschriftung', 'ub' ),
					'default' => '#fff',
				),
				'form_button_border'            => array(
					'type'    => 'color',
					'label'   => __( 'Schaltflächenrahmen', 'ub' ),
					'default' => '#006799',
				),
				'form_button_background'        => array(
					'type'    => 'color',
					'label'   => __( 'Schaltflächenhintergrund', 'ub' ),
					'default' => '#0085ba',
				),
				'form_background'               => array(
					'type'    => 'color',
					'label'   => __( 'Formularhintergrund', 'ub' ),
					'default' => '#fff',
					'data'    => array(
						'alpha' => true,
					),
				...
				'form_shadow'                   => array(
					'type'    => 'color',
					'label'   => __( 'Formularbox Schatten', 'ub' ),
				...
				'form_container_background'     => array(
					'type'    => 'color',
					'label'   => __( 'Formularbox Hintergrund', 'ub' ),
					'default' => 'transparent',
					'data'    => array(
						'alpha' => true,
					),
		...
		protected function get_options_fields_colors_links_below_form( $defaults = array() ) {
			$data = array(
				...
				'links_below_form_back'            => array(
					'type'    => 'color',
					'label'   => __( 'Link "Zurück zur Website"', 'ub' ),
					'default' => '#555d66',
				),
				'links_below_form_policy'          => array(
					'type'    => 'color',
					'label'   => __( 'Datenschutzerklärung', 'ub' ),
					'default' => '#0073aa',
					'panes'   => array(
						'end_pane' => true,
					),
		...
		public function get_template_configuration() {
			...
			$args     = array(
				...
				'sui'   => false,
				'class' => 'pstoolkit-login-screen-choose-template',
				'data'  => array(
			...
			$args     = array(
				...
				'classes' => array(
					'pstoolkit-login-screen-choose-template-dialog',
		...
		public function ajax_set_template() {
			...
			if ( 'start-from-scratch' === $id ) {
				...
				$message = array(
					'type'    => 'success',
					'message' => __( 'Du kannst jetzt von vorne anfangen!', 'ub' ),
				);
			...
			if ( is_array( $result ) && ! empty( $result ) ) {
				$message = array(
					'type'    => 'success',
					'message' => sprintf(
						__( 'Die Themenkonfiguration "%s" wurde erfolgreich geladen!', 'ub' ),
		...
		private function convert_old_themes_path( $path ) {
			$re = '@pstoolkit-files/modules/custom-login-screen/themes@';
		...
		public function add_login_header() {
			...
			echo '<div class="pstoolkit-login">';
		}
		...
		private function get_ace_selectors() {
			$selectors = array(
				'general' => array(
					'selectors' => array(
						'.pstoolkit-login' => __( 'Container', 'ub' ),
						'h1'            => __( 'Logo', 'ub' ),
						'#login'        => __( 'Formular Wrapper', 'ub' ),
						'#loginform'    => __( 'Formular', 'ub' ),
					),
				),
			);
			return $selectors;
		...
		public function get_options_fields_design_message( $defaults = array() ) {
			$data = array(
				'message' => array(
					...
					'label'     => __( 'Typ', 'ub' ),
					'options'   => array(
						'none'    => __( 'Informieren', 'ub' ),
						'success' => __( 'Erfolg', 'ub' ),
					),
					'default'   => 'none',
					'accordion' => array(
						'begin' => true,
		...
		public function get_options_fields_design_container( $defaults = array() ) {
			$data = array(
				'container_width'          => array(
					...
					'accordion' => array(
						'begin' => true,
						'title' => __( 'Formular Container', 'ub' ),
					),
					'group'     => array(
						'begin' => true,
					),
		...
		public function validate_password( $wp_error ) {
			...
			if ( empty( $password_1 ) ) {
				...
				$wp_error->add( 'password_1', __( '<strong>FEHLER</strong>: Bitte gib in beide Passwortfelder das gleiche Passwort ein.', 'ub' ) );
			}
			return $wp_error;
		}
		...
		public function validate_password_wpmu( $results ) {
			...
			if ( empty( $password_1 ) ) {
				...
				$results['errors']->add( 'password_1', __( '<strong>FEHLER</strong>: Bitte gib in beide Passwortfelder das gleiche Passwort ein.', 'ub' ) );
			}
			return $results;
		}
	}
}
new PSToolkit_Login_Screen();
